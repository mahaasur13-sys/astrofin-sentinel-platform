...
class ObjectiveWeights:
    ...
    def utility(
        self, throughput: float, reliability: float, latency: float, energy: float
    ) -> float:
        return (
...
class ObjectiveReweighter:
    ...
    def update(self, regret_signal: float, performance: dict) -> ObjectiveWeights:
        ...
        for i, key in enumerate(["alpha", "beta", "gamma", "delta"]):
            self._gradient_accum[key] = (
                self.momentum * self._gradient_accum[key] + gradient[i]
            )

        # Gradient ascent
        new_weights = ObjectiveWeights(
            alpha=max(
                0.05, self._weights.alpha + self.lr * self._gradient_accum["alpha"]
            ),
            beta=max(0.05, self._weights.beta + self.lr * self._gradient_accum["beta"]),
            gamma=max(
                0.0, self._weights.gamma + self.lr * self._gradient_accum["gamma"]
            ),
            delta=max(
                0.0, self._weights.delta + self.lr * self._gradient_accum["delta"]
            ),
