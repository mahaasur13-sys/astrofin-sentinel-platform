...
async def run_sentinel_v5(
    ...
    from core.logging import set_correlation_id, new_correlation_id
...
def main():
    ...
    parser.add_argument("--mode", default="paper", choices=["historical", "paper", "live"])
    parser.add_argument("--karl", action="store_true", default=True)
    parser.add_argument("--query", default="", help="User query")
    args = parser.parse_args()

    if args.karl:
        result = asyncio.run(run_karl_sentinel_v5(user_query=args.query, symbol=args.symbol, timeframe=args.timeframe))
    else:
        result = asyncio.run(run_sentinel_v5(user_query=args.query, symbol=args.symbol, timeframe=args.timeframe))
