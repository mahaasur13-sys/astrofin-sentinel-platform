...
register_role("root", [c.value for c in Capability], "Unrestricted — used only by governance kernel")

register_role("ml_inference", ["infra:read", "ml:read", "state:read"], "ML inference pipeline — read-only access")
...
class ExecutionContext:
    ...
    def create(trace_id: str, role: str, session_id: str) -> "ExecutionContext":
        ...
        return ExecutionContext(trace_id=trace_id, role=role, session_id=session_id, granted=cap_set.capabilities)
