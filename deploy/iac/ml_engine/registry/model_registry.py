...
class ModelRegistry:
    ...
    def _compute_hash(self, params: dict, metrics: dict) -> str:
        ...
        h.update(json.dumps({k: float(v) for k, v in metrics.items()}, sort_keys=True).encode())
    ...
    def list_versions(self, model_name: str | None = None) -> list[dict[str, Any]]:
        ...
        return sorted(self._index.values(), key=lambda v: v["registered_at"], reverse=True)
