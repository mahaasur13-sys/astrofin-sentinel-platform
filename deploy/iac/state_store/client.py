...
class StateStore:
    ...
    def __init__(self, host="localhost", port=5432, dbname="clusterdb", user=None, password=None):
        self.host = host
        self.port = port
        self.dbname = dbname
    ...
    def _cursor(self, dict_cursor=True):
        cur = self._conn.cursor(cursor_factory=RealDictCursor if dict_cursor else None)
        try:
            yield cur
            self._conn.commit()
        ...
        with self._cursor() as cur:
            cur.execute(
                ...
                ON CONFLICT (hostname) DO UPDATE SET
                    ...
                    last_seen = NOW()
                """,
                (hostname, roles, gpu_count, gpu_model, cpu_cores, memory_gb),
            )
    ...
    # Events (append-only)
    # -------------------------------------------------------------------------
    def _write_event(self, job_id: str, event_type: str, payload: dict[str, Any]) -> None:
        with self._cursor() as cur:
            cur.execute(
    ...
    # -------------------------------------------------------------------------
    def write_scheduler_decision(self, job_id: str, round_num: int, scores: list[dict], selected_node: str) -> None:
        with self._cursor() as cur:
    ...
    # Admission decisions
    # -------------------------------------------------------------------------
    def write_admission_decision(self, job_id: str, decision: str, reason: str, cluster_util: dict) -> None:
        with self._cursor() as cur:
            cur.execute(
    ...
    def write_failure_recovery(
        ...
        with self._cursor() as cur:
            cur.execute(
                ...
                (job_id, hostname, failure_type, recovery_action, attempt, success),
            )
    ...
    def get_cluster_state(self) -> dict:
        with self._cursor() as cur:
            ...
            rows = cur.fetchall()
        return {"nodes": [dict(r) for r in rows], "ts": datetime.utcnow().isoformat()}
