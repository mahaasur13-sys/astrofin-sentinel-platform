...
def get_tunnel_status() -> dict:
    try:
        result = subprocess.run(
            ["ip", "addr", "show", "wg0"], capture_output=True, text=True, timeout=5
        )
...
def cmd_status() -> int:
    ...
    print(
        f"  {'✅' if tunnel['up'] else '❌'} AmneziaWG Tunnel (wg0): {'UP' if tunnel['up'] else 'DOWN'}"
    )
    # VictoriaMetrics
    try:
...
def cmd_switch(backend: str) -> int:
    if backend not in BACKENDS:
        print(
            f"ERROR: Unknown backend '{backend}'. Available: {', '.join(BACKENDS.keys())}"
        )
        return 1
    cfg = load_config()
...
def main() -> int:
    ...
    if cmd == "status":
        ...
        if len(sys.argv) < 3:
            print("Usage: acos monitor switch <backend>")
            return 1
