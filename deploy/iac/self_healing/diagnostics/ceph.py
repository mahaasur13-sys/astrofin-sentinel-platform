...
def get_ceph_status_detail(host: str | None = None) -> CephStatus:
    ...
    if health_out:
        ...
        for line in lines:
            if any(
                kw in line.lower()
                for kw in ["heartbeat", "no reply", "osd. down", "mon. down"]
            ):
...
def detect_split_brain(status: CephStatus) -> tuple[bool, str]:
    ...
    if len(status.quorum_names) == 1:
        reasons.append(
            f"Single MON quorum [{status.quorum_names[0]}] → split-brain risk if any network glitch"
        )

    # 2. PGs stale + unclean simultaneously (split-brain aftermath)
    if status.pgs_stale and status.pgs_unclean:
        reasons.append(
            f"Split-brain: {len(status.pgs_stale)} stale PGs + {len(status.pgs_unclean)} unclean PGs"
        )

    # 3. Inconsistent PGs (data divergent)
    if status.pgs_inconsistent:
        reasons.append(
            f"DATA INCONSISTENCY: {len(status.pgs_inconsistent)} inconsistent PGs"
        )

    # 4. Explicit heartbeat issues
    if len(status.heartbeat_issues) >= 2:
        reasons.append(
            f"Multiple heartbeat failures: {len(status.heartbeat_issues)} issues"
        )
