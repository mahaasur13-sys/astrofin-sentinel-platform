...
class MockNode:
    def __init__(self, hostname, gpu_count=1, gpu_load=20.0, cpu_load=30.0, mem_gb=32, mem_used=10.0, status="HEALTHY"):
...
def test_same_seed_same_node():
    ...
    assert all(r == "rtx-node" for r in results), f"Determinism broken: got {results}"
...
def test_gpu_load_changes_ranking():
    ...
    best, scores = score_and_select(job, store)
    assert best.hostname == "rtx-cool", f"Expected rtx-cool (lower GPU load), got {best.hostname}"
...
def test_failure_penalty_affects_score():
    ...
    store.get_recent_failures = lambda m: [{"node_hostname": "flaky", "failure_type": "GPU_OOM"} for _ in range(3)]

    job = MockJob("gpu")
    best, scores = score_and_select(job, store)

    # flaky should NOT win despite lower GPU load (failure penalty = 3*20=60)
    assert best.hostname == "stable", f"Expected stable (failure penalty on flaky), got {best.hostname}"
...
def test_backpressure_throttles_queue():
    ...
    assert result.decision == AdmitDecision.QUEUED, f"Expected QUEUED under backpressure, got {result.decision}"
