...
class SyntheticScheduler:
    ...
    def _run_simulation(self) -> InjectionResult:
        ...
        return InjectionResult(
            ...
            p50_wait_sec=(
                float(sorted(wait_times)[len(wait_times) // 2]) if wait_times else 0
            ),
            p99_wait_sec=(
                float(sorted(wait_times)[int(len(wait_times) * 0.99)])
                if len(wait_times) > 10
                else 0
            ),
            p50_runtime_sec=(
                float(sorted(runtimes)[len(runtimes) // 2]) if runtimes else 0
            ),
            p99_runtime_sec=(
                float(sorted(runtimes)[int(len(runtimes) * 0.99)])
                if len(runtimes) > 10
                else 0
            ),
