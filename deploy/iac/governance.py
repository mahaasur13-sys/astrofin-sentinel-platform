...
LAYER_BY_DIR = {
    ...
    "incident": "v8",
    "admission": "v8",
    "constraint_compiler": "v8",
    "k8s_manifests": "v8",
    "v7": "v7",
    "policy_governor": "v7",
    "drift_alignment": "v7",
    "budget_controller": "v7",
    "adversarial_sim": "v7",
    "meta_learner": "v7",
    "objective_reweight": "v7",
    "v6": "v6",
    "constraint_engine": "v6",
    "solver": "v6",
    "policy_eval": "v6",
    "digital_twin": "v6",
    "ml_engine": "v5",
...
def adversarial_analysis(violations, reachability, layer_edges, dynamic_findings, lt_se, fp_im):
    ...
    if len(lt_se) >= 3:
        score -= 0.2
        findings.append(f"{len(lt_se)} lt side effects")
    if len(fp_im) >= 3:
        score -= 0.1
        findings.append(f"{len(fp_im)} fp impure modules")
...
def run():
    ...
    if sev["critical"]:
        ...
        status = "BLOCK"
        risk = "HIGH"
    elif violations or lt_side_effects or fp_impure:
        status = "WARN"
        risk = "MEDIUM"
    else:
        status = "PASS"
    ...
    return {
        ...
        "FIX_APPLIED": None,
        "CONFIDENCE": {
            "static_analysis": static_conf,
            "runtime_validation": runtime_conf,
