...
def export_agent(agent_name: str, output_dir: str = "./exported_agents") -> dict[str, Any]:
    ...
    try:
        from AstroFinSentinelV5.agents import (
            astro_council_agent, fundamental_agent, quant_agent, macro_agent,
            technical_agent, bull_researcher, bear_researcher, risk_agent,
            sentiment_agent, synthesis_agent
        )
        agent_map = {
...
def main():
    ...
    mcp_recommend_parser = subparsers.add_parser("mcp-recommended", help="Show recommended MCP servers")
