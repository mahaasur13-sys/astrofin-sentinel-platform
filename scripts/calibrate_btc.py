...
async def main():
    ...
    def pf(d):
        return round(d["wins"] / d["losses"], 2) if d.get("losses", 0) > 0 else float("inf")

    for r in results:
        wr = r["win_rate"] * 100
        print(
            f"{r['label']:<12} {r['trades']:>8} {wr:>7.1f}% {r['max_drawdown_pct']:>7.1f}% {r['total_return_pct']:>7.1f}% {pf(r):>8.2f}"
        )
    ...
    with open(report_path, "w") as f:
        f.write("# HMM-KARL Calibration Report\n\n")
        f.write("| Config | Trades | WinRate | MaxDD | Return | PF |\n")
        f.write("|--------|--------|---------|-------|--------|----|\n")
        for r in results:
            f.write(
                f"| {r['label']} | {r['trades']} | {r['win_rate'] * 100:.1f}% | {r['max_drawdown_pct']:.1f}% | {r['total_return_pct']:.1f}% | {pf(r):.2f} |\n"
            )
        f.write(
