...
def _solar_event(dt, lat, lon, is_sunrise):
    ...
    ar = _m.radians(ga)
    eqc = (
        _m.sin(ar) * (1.914602 - jc * (0.004817 + 0.000014 * jc))
        + _m.sin(2 * ar) * (0.019993 - 0.000101 * jc)
        + _m.sin(3 * ar) * 0.000289
    )
    tl = gl + eqc
    ...
    et = 4 * _m.degrees(
        yf * _m.sin(2 * _m.radians(gl))
        - 2 * ec * _m.sin(ar)
        + 4 * ec * yf * _m.sin(ar) * _m.cos(2 * _m.radians(gl))
        - 0.5 * yf**2 * _m.sin(4 * _m.radians(gl))
        - 1.25 * ec**2 * _m.sin(2 * ar)
    )
    zen = 90.833
    cha = (_m.cos(_m.radians(zen)) - _m.sin(_m.radians(lat)) * _m.sin(_m.radians(decl))) / (
        _m.cos(_m.radians(lat)) * _m.cos(_m.radians(decl))
    )
...
if __name__ == "__main__":
    ...
    print("  1. Kala    05:35–07:18  (Loss)")
    print("  2. Shubh   07:18–09:01  (Good)")
    print("  3. Roga    09:01–10:44  (Evil)")
    print("  4. Udveg   10:44–12:27  (Bad)")
    print("  5. Chara   12:27–14:10  (Neutral)")
    print("  6. Labh    14:10–15:54  (Gain)")
    print("  7. Amrit   15:54–17:37  (Best)")
    print("  8. Kala    17:37–19:20  (Loss)")
    ...
    print(f"\n{'═' * 60}")
    print("  Choghadiya: Samara (UTC+4), Saturday 2026-07-18")
    print(f"{'═' * 60}")
    ...
    stop_periods = [
        p for p in day_choghadiya(dt_samara) + night_choghadiya(dt_samara) if QUALITY.get(p["name"]) == "inauspicious"
    ]
    if stop_periods:
