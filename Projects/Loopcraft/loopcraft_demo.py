...
class Environment:
    ...
    def __init__(self, optimal_lr: float = LANDSCAPE_OPTIMAL_LR, noise_std: float = LANDSCAPE_NOISE_STD, seed: int | None = None) -> None:
    ...
    try:
        for i in range(start_iter, start_iter + max_iterations):
            ...
            try:
                ...
                state.attempts.append(Attempt(iteration=i, lr=proposed_lr, loss=float("inf"), accepted=False, reason=f"rejected: env error: {e}").__dict__)
            ...
            state.attempts.append(Attempt(iteration=i, lr=proposed_lr, loss=loss, accepted=accepted, reason=reason).__dict__)
            ...
            if verbose:
                ...
                print(f"[ITER {i:03d}] {tag} | lr={proposed_lr:.6f} loss={loss:.6f} | best_lr={state.best_lr:.6f} best_loss={state.best_loss:.6f}")
                if accepted:
...
def main() -> int:
    ...
    parser.add_argument("--iterations", type=int, default=DEFAULT_MAX_ITERATIONS, help=f"макс. итераций (по умолчанию {DEFAULT_MAX_ITERATIONS})")
