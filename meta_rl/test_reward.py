...
class TestRewardConfig:
    ...
    def test_default_construction_emits_no_warning(self):
        ...
        assert reward_warnings == [], (
            f"Default RewardConfig unexpectedly warned: {[str(w.message) for w in reward_warnings]}"
        )
    ...
    def test_within_tolerance_no_warning(self):
        ...
        reward_warnings = [w for w in caught if "RewardConfig" in str(w.message)]
        assert reward_warnings == [], (
            f"Sum 1.005 (within tolerance) should not warn, got: {[str(w.message) for w in reward_warnings]}"
        )
