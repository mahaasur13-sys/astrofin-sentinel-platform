...
> Полные зеркала: `astrofin-sentinel-v5/`, `[ARCHIVED] audit_repo/`, `push/`, `agents/`, `_pr_logs/`, `hived/`.  
...
|---|---|---|
| **Один канонический корень проекта** | 4+ зеркала: `AstroFinSentinelV5/`, `astrofin-sentinel-v5/`, `push/`, `[ARCHIVED] audit_repo/`, `agents/`, `_pr_logs/`, `hived/` | Один `src/` или `app/` |
...
2. **Расширить `.graphifyignore`** — добавить `astrofin-sentinel-v5/`, `[ARCHIVED] audit_repo/`, `push/`, `_pr_logs/`, `hived/`, `hived-asur/`, `_sbs_old/`, `tests/`-без-префикса, `agents/`-в-корне, `trading/`/`core/`/`meta_rl/`/`backtest/`-в-корне. Тогда граф будет **только по активному коду**.
...
| Зеркала проекта | 6+ (`AstroFinSentinelV5`, `astrofin-sentinel-v5`, `push`, `[ARCHIVED] audit_repo`, `agents`, `_pr_logs`, `hived`, `_sbs_old`) |
