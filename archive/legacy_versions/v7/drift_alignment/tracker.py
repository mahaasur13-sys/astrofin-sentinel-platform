...
class DriftAlignment:
    ...
    def record_signals(self, signals: DriftSignals) -> None:
        ...
        self._feature_history = [f for f, m in zip(self._feature_history, mask, strict=False) if m]
        self._model_error_history = [m for m, mk in zip(self._model_error_history, mask, strict=False) if mk]
        self._system_drift_history = [s for s, m in zip(self._system_drift_history, mask, strict=False) if m]
        self._timestamps = [t for t, m in zip(self._timestamps, mask, strict=False) if m]
    ...
    def measure_system_drift(
        ...
        divergences = []
        for sim, act in zip(simulated_states[-50:], actual_states[-50:], strict=False):
