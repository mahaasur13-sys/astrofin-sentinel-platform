...
async def main():
    log.info("=" * 60)
    log.info("Phase 5.3: RAG Integration Smoke Test")
    log.info("=" * 60)
    ...
    log.info(f"  ✅ Singleton: {len(rag1.chunks)} chunks loaded, same instance: {rag1 is rag2}")
    ...
    for i, c in enumerate(chunks):
        log.info(f"  [{i+1}] rrf={c.metadata.get('rrf_score', 0):.4f} src={c.metadata.get('source', '?')[:60]}")
    ...
    log.info(f"  ✅ RAG context: {len(ctx)} chars")

    # Test 4: Second call (no model reload)
    log.info("\n[Test 4] Second call (cached model)...")
    ctx2 = agent._get_rag_context("Ethereum market outlook")
    log.info(f"  ✅ Context length: {len(ctx2)} chars")
    assert len(ctx2) > 0

    log.info("\n" + "=" * 60)
    log.info("✅ ALL TESTS PASSED — Phase 5.3 RAG Integration")
    log.info("=" * 60)
