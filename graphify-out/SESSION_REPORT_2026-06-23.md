...
|---|---|
| `graph.json` | 38,682 nodes, 6,074 links, 11 relation types |
| `inferred_clean.jsonl` | 500 edges, **monomorphic** (100% `calls`) |
| `infer_edges.py` | `recall_score = tier * decay * conf` (no relation axis) |
| Override pairs | 7/7 defined in `config/memory_overrides.json` |
| ADR | 0004 (override mechanism) only; no 0005 |

## 3. Root cause of the monomorphic sample

`inferred_clean.jsonl` was not "monomorphic because `infer_edges.py` only
...
the highest-weight relation in `graph.json`). `infer_edges.py` preserves
...
### 4.1 New selector — `select_top_inferred.py`

Built a 3-pass selector over `graph.json`:
...
### 4.2 Updated sampler — `build_balanced_sample.py`
...
- Added `--relation-weights <variant>` CLI flag reading
  `config/relation_weights.json`.
- `relation_variant` and `relation_weights_path` added to summary.

### 4.4 New healthcheck — `graphify-out/healthcheck.py`

6 checks (c1–c6) → 8 checks (c1–c8):

- c1: `graph.json` structure
- c2: `inferred_clean.jsonl` exists with ≥7 relation types
- c3: enriched file exists with ≥7 relation types + `recall_score`
- c4: 7/7 override pairs survive `graph.json` → `inferred_clean` → enriched
- c5: `recall_test.py` runs clean
...
`inferred_clean.jsonl` sample (166 edges, 7 override GT):
...
| `3d7c6cf` | graphify-out: relation-weights config + A/B harness (--relation-weights CLI, ab_*.json, ab_relation_weights.md) |
