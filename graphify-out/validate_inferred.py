...
SAMPLE = Path(os.environ.get("INFERRED_SAMPLE") or (sys.argv[1] if len(sys.argv) > 1 else "/tmp/inferred_sample_500.json"))
...
def main() -> None:
    ...
    lines.insert(7, "")
    lines.insert(7, f"**Verdict summary (N={total}):** " + ", ".join(f"`{k}`={v} ({v * 100 / total:.0f}%)" for k, v in sorted(stats.items(), key=lambda x: -x[1])))
    lines.insert(7, "")
