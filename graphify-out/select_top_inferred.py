...
def main() -> None:
    ...
    ap.add_argument("--min-per-relation", type=int, default=10, help="Floor per relation bucket when enough edges exist (default: 10).")
    ap.add_argument(
        "--hard-cap-per-relation",
        type=int,
        default=200,
        help="Absolute ceiling per relation across ALL passes (default: 200). Defends against large --max (e.g. 5000) collapsing diversity when a few buckets (calls/imports/uses/inherits) dominate.",
    )
    ap.add_argument("--cross-file-priority", action="store_true", default=True, help="Prefer cross-file edges inside each bucket (default: on).")
