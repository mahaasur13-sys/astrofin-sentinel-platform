...
def process_amre(
    ...
    from .audit import EnsembleSelection as _ES
    from .audit import MarketSnapshot as _MS
    ...
    decision_record = build_decision_record(
        ...
        regime=state.get("regime", "NORMAL"),
        state_hash=hashlib.md5(f"{symbol}:{state.get('current_price', 0)}:{state.get('regime', 'NORMAL')}".encode()).hexdigest()[:12] if "hashlib" in dir() else "0" * 12,  # nosec B324 — content hash for cache key, not security
