...
def create_k8s_job_object(romatask: Dict[str, Any]) -> Dict[str, Any]:
    ...
    job_manifest = {
        ...
        "metadata": {
            ...
            "labels": {"app": "roma", "romatask-name": romatask["metadata"]["name"], "gpu-required": str(gpu_required).lower()},
        },
        "spec": {
            "backoffLimit": 0,
                    ...
                    "containers": [
                        {
                            ...
                            "resources": {"limits": {"nvidia.com/gpu": "1"} if gpu_required else {}, "requests": {"nvidia.com/gpu": "1"} if gpu_required else {}} if gpu_required else {},
                        }
                    ],
                }
            },
...
def update_romatask_status(api: client.ApiClient, romatask: Dict[str, Any], phase: str, job_name: Optional[str] = None, error: Optional[str] = None):
    ...
    try:
        body = {"status": {"phase": phase, "jobId": romatask.get("status", {}).get("jobId", ""), "updatedAt": datetime.utcnow().isoformat() + "Z"}}
        if job_name:
        ...
        client.CustomObjectsApi(api).patch_namespaced_custom_object_status(
            group="roma.ai", version="v1", namespace=romatask["metadata"]["namespace"], plural="romatasks", name=romatask["metadata"]["name"], body=body
        )
