...
class JobStore:
    ...
    def submit(self, plugin: str, payload: Dict[str, Any], gpu: float = 1.0, ttl: float = 3600.0, max_retries: int = 3) -> Job:
        with self._lock:
            self._seq += 1
            job = Job(id=f"job-{self._seq:05d}-{uuid.uuid4().hex[:8]}", plugin=plugin, payload=payload, gpu_allocated=gpu, ttl_seconds=ttl, max_retries=max_retries)
