...
async def stripe_webhook(
    ...
    try:
        ...
        if etype in ("checkout.session.completed", "customer.subscription.created"):
            ...
            ledger.record_usage(tid, amount, obj.get("currency", "usd"), f"Invoice {obj.get('id')}", obj.get("period_end", 0))
            share = calc.calculate(tid, amount)
            if share["revenue_share_cents"] > 0:
                ledger.record_revenue_share(tid, share["revenue_share_cents"], obj.get("id"), share["revenue_share_percent"])
            ok = True
