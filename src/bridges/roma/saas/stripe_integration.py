...
class StripeIntegration:
    ...
    def create_customer(self, org_id: str, email: str) -> StripeCustomer:
        c = StripeCustomer(id=f"cus_{uuid.uuid4().hex[:14]}", org_id=org_id, email=email, stripe_customer_id=f"cus_stripe_{uuid.uuid4().hex[:8]}")
    ...
    def create_subscription(self, org_id: str, tier: str) -> Subscription:
        now = time.time()
        s = Subscription(id=f"sub_{uuid.uuid4().hex[:12]}", org_id=org_id, tier=tier, status=SubscriptionStatus.ACTIVE, current_period_start=now, current_period_end=now + 30 * 24 * 3600)
    ...
    def record_usage(self, org_id: str, key_id: str, gpu_seconds: float, cost: float, job_id: str = "", plugin: str = "") -> UsageRecord:
        rec = UsageRecord(id=f"usr_{uuid.uuid4().hex[:12]}", org_id=org_id, key_id=key_id, gpu_seconds=gpu_seconds, cost=cost, job_id=job_id, plugin=plugin)
    ...
    def generate_invoice(self, org_id: str) -> Invoice:
        ...
        inv = Invoice(
            ...
            org_id=org_id,
            items=[{"desc": r.plugin or "GPU compute", "gpu_s": r.gpu_seconds, "cost": r.cost} for r in usage],
...
class WebhookSimulator:
    ...
    def simulate_payment_success(self, org_id: str, invoice_id: str, amount_cents: int):
        e = {"type": "payment_intent.succeeded", "org_id": org_id, "invoice_id": invoice_id, "amount_cents": amount_cents}
    ...
    def simulate_subscription_created(self, org_id: str, tier: str):
        ...
        e = {"type": "customer.subscription.created", "org_id": org_id, "sub_id": s.id, "tier": tier}
        self._events.append(e)
        return e
