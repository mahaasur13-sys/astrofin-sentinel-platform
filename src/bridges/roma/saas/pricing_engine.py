...
class PricingEngine:
    def quote(self, gpu_seconds, gpu_model="RTX4090", tier="pro", region_mult=1.0, cluster_load=0.5):
        ...
        if tier_mult == 0:
            return PriceQuote(
                ...
                total_cost=0.0,
                breakdown={"tier": tier, "quota_free": True, "monthly_quota_hrs": TIER_QUOTA.get(PricingTier(tier), 0) // 3600, "gpu_per_hour": gpu_rate * 3600},
...
class ProfitCalculator:
    ...
    def margin(self, quote: PriceQuote) -> Dict[str, Any]:
        ...
        return {"revenue": round(rev, 4), "cost": round(our_cost, 4), "profit": round(profit, 4), "GPM": round(gpm, 1), "markup": round(rev / our_cost, 2) if our_cost > 0 else 0}
