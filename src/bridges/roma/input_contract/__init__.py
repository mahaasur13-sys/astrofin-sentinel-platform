...
class ValidationResult:
    ...
    def to_api_response(self) -> dict:
        return {"status": "rejected", "error": {"code": self.code, "message": self.message, "severity": self.severity}}
...
class InputContractValidator:
    ...
    REJECTED_EMPTY_INPUT = ValidationResult(valid=False, code="USER_TASK_REQUIRED", message="Task input cannot be empty. Provide a valid task description.", severity="critical", rejected_input=None)

    REJECTED_TOO_SHORT = ValidationResult(valid=False, code="USER_TASK_TOO_SHORT", message="Task description too short (minimum 3 characters).", severity="high", rejected_input=None)
    ...
    def validate(cls, user_task: Optional[str]) -> ValidationResult:
        ...
        if dangerous:
            return ValidationResult(valid=False, code="USER_TASK_DANGEROUS_PATTERN", message=f"Dangerous pattern detected: {dangerous}. Task rejected.", severity="high", rejected_input=stripped[:50])

        # VALID
        return ValidationResult(valid=True, code="USER_TASK_VALID", message="Task accepted", severity="low", rejected_input=None)
