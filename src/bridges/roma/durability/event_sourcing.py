...
class Event:
    ...
    def create(event_type: EventType, aggregate_id: str, tick: int, payload: Dict) -> "Event":
        return Event(event_id=str(uuid.uuid4()), event_type=event_type, aggregate_id=aggregate_id, tick=tick, timestamp_ns=time.time_ns(), payload=payload, metadata={})
...
class EventStore:
    ...
    def append(self, event: Event) -> None:
        ...
        with self._lock:
            self._conn.execute(
                ...
                (event.event_id, event.event_type.value, event.aggregate_id, event.tick, event.timestamp_ns, json.dumps(event.payload), json.dumps(event.metadata)),
            )
            self._conn.commit()
    ...
    def append_batch(self, events: List[Event]) -> None:
        ...
        with self._lock:
            self._conn.executemany(
                "INSERT INTO events (event_id, event_type, aggregate_id, tick, timestamp_ns, payload, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [(e.event_id, e.event_type.value, e.aggregate_id, e.tick, e.timestamp_ns, json.dumps(e.payload), json.dumps(e.metadata)) for e in events],
            )
            self._conn.commit()
    ...
    def get_events_for_aggregate(self, aggregate_id: str) -> List[Event]:
        ...
        with self._lock:
            rows = self._conn.execute("SELECT event_id, event_type, aggregate_id, tick, timestamp_ns, payload, metadata FROM events WHERE aggregate_id = ? ORDER BY tick", (aggregate_id,)).fetchall()
            return [self._row_to_event(r) for r in rows]

    def get_events_since(self, tick: int, limit: int = 1000) -> List[Event]:
        """Get events from tick N onwards (for replay)."""
        with self._lock:
            rows = self._conn.execute("SELECT event_id, event_type, aggregate_id, tick, timestamp_ns, payload, metadata FROM events WHERE tick >= ? ORDER BY tick LIMIT ?", (tick, limit)).fetchall()
            return [self._row_to_event(r) for r in rows]
    ...
    def _row_to_event(self, row) -> Event:
        return Event(event_id=row[0], event_type=EventType(row[1]), aggregate_id=row[2], tick=row[3], timestamp_ns=row[4], payload=json.loads(row[5]), metadata=json.loads(row[6]))
...
class JobProjection:
    ...
    def build_state(self, job_id: str) -> JobState:
        ...
        state = JobState(job_id=job_id, status="unknown", priority=0, vram_gb=0.0, gpu_node=None, created_tick=events[0].tick, last_tick=events[0].tick)
...
class JobAggregate:
    ...
    def submit_job(self, job_id: str, priority: int, vram_gb: float, gpu_required: bool) -> Event:
        ...
        event = Event.create(EventType.JOB_SUBMITTED, job_id, tick, {"priority": priority, "vram_gb": vram_gb, "gpu_required": gpu_required})
...
class DeterministicReplay:
    ...
    def replay_and_verify(self, start_tick: int, apply_fn: Callable[[Event], None]) -> Dict[str, Any]:
        ...
        return {"events_replayed": len(events), "events_applied": len(applied_ticks), "errors": errors, "deterministic": len(errors) == 0}
