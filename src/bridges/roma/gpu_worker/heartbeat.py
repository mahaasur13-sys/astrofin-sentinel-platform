...
class HeartbeatClient:
    ...
    def _probe_gpu(self) -> dict:
        ...
        try:
            ...
            result = subprocess.run(["nvidia-smi", "--query-gpu=utilization.gpu,memory.used,memory.total", "--format=csv,noheader,nounits"], capture_output=True, text=True, timeout=5)
            if result.returncode == 0:
                util, used, total = result.stdout.strip().split(", ")
                return {"gpu_util": float(util) / 100.0, "vram_used_gb": float(used) / 1024.0, "vram_total_gb": float(total) / 1024.0}
        except Exception:
            pass
        # Mock for testing
    ...
    def report_job_start(self, job_id: str):
        ...
        try:
            requests.post(f"{self.roma_url}/worker/job/start", json={"worker_id": self.worker_id, "job_id": job_id}, timeout=3)
        except Exception:
            pass
    ...
    def report_job_complete(self, job_id: str, success: bool):
        ...
        try:
            requests.post(f"{self.roma_url}/worker/job/complete", json={"worker_id": self.worker_id, "job_id": job_id, "success": success}, timeout=3)
        except Exception:
            pass
