...
class PluginResult:
    ...
    def to_dict(self) -> Dict:
        return {"success": self.success, "output": self.output, "error": self.error, "metrics": self.metrics, "timestamp": self._timestamp}
...
class MLTrainingPlugin(IPlugin):
    ...
    async def on_execute(self, task: ROMATask, context: IExecutionContext) -> PluginResult:
        ...
        if context.gpu_available and context.vram_gb >= gpu_mem:
            return PluginResult(
                success=True, output={"model_trained": True, "epochs_completed": epochs, "gpu_used": context.node_name, "vram_gb": gpu_mem}, metrics={"vram_gb": gpu_mem, "duration_s": duration}
            )
...
class InferencePlugin(IPlugin):
    ...
    async def on_execute(self, task: ROMATask, context: IExecutionContext) -> PluginResult:
        ...
        return PluginResult(success=True, output={"inference_id": task.task_id, "model": model, "node": context.node_name})
