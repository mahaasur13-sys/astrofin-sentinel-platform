...
def build_namespace(tenant_id: str, labels: dict) -> dict:
    ns_manifest = {
        "apiVersion": "v1",
        "kind": "Namespace",
        "metadata": {"name": f"roma-{tenant_id}", "labels": {"roma.io/tenant": tenant_id, "app.kubernetes.io/managed-by": "roma-tenant-controller", **labels}},
    }
    return ns_manifest
...
def build_configmap(tenant_id: str, branding: dict) -> dict:
    cm = {
        "apiVersion": "v1",
        "kind": "ConfigMap",
        "metadata": {"name": f"roma-{tenant_id}-branding", "namespace": f"roma-{tenant_id}", "labels": {"roma.io/tenant": tenant_id}},
        "data": {
...
def build_api_secret(tenant_id: str, api_key: str, secret_name: str = "api-key") -> dict:
    ...
    return {
        "apiVersion": "v1",
        "kind": "Secret",
        "metadata": {"name": secret_name, "namespace": f"roma-{tenant_id}", "labels": {"roma.io/tenant": tenant_id}},
        "type": "Opaque",
        "stringData": {"api-key": api_key, "tenant-id": tenant_id},
    }
...
def build_ingress(tenant_id: str, domain: str, subdomain: str, namespace: str) -> dict:
    ...
    return {
        ...
        "spec": {
            ...
            "tls": [{"hosts": [f"{subdomain}.{domain}", domain], "secretName": f"roma-{tenant_id}-tls"}],
            "rules": [{"host": f"{subdomain}.{domain}", "http": {"paths": [{"path": "/", "pathType": "Prefix", "backend": {"service": {"name": f"roma-{tenant_id}-api", "port": {"number": 8080}}}}]}}],
        },
    }
...
def build_certificate(tenant_id: str, domain: str, subdomain: str, namespace: str) -> dict:
    ...
    return {
        ...
        "kind": "Certificate",
        "metadata": {"name": f"roma-{tenant_id}-cert", "namespace": namespace, "labels": {"roma.io/tenant": tenant_id}},
        "spec": {"secretName": f"roma-{tenant_id}-tls", "issuerRef": {"name": CERT_CLUSTER_ISSUER, "kind": "ClusterIssuer"}, "dnsNames": [f"{subdomain}.{domain}", domain]},
    }
...
def build_kong_consumer(tenant_id: str, api_key: str) -> dict:
    ...
    return {
        ...
        "kind": "KongConsumer",
        "metadata": {"name": tenant_id, "namespace": KONG_NAMESPACE, "annotations": {"konghq.com/plugins": f"rate-limit-{tenant_id}"}},
        "username": tenant_id,
...
def build_kong_credential(tenant_id: str, api_key: str) -> dict:
    ...
    return {
        "apiVersion": "configuration.konghq.com/v1",
        "kind": "KongCredential",
        "metadata": {"name": f"{tenant_id}-credential", "namespace": KONG_NAMESPACE},
        "type": "key-auth",
...
def build_rate_limit_plugin(tenant_id: str, plan: str) -> dict:
    ...
    limits = {"free": {"minute": 60, "hour": 500, "day": 5000}, "pro": {"minute": 300, "hour": 5000, "day": 50000}, "enterprise": {"minute": 3000, "hour": 50000, "day": 500000}}
    pl = limits.get(plan, limits["free"])
    return {
        "apiVersion": "configuration.konghq.com/v1",
        "kind": "KongPlugin",
        "metadata": {"name": f"rate-limit-{tenant_id}", "namespace": ROMA_SYSTEM_NS},
        "plugin": "rate-limiting",
        "config": {"minute": pl["minute"], "hour": pl["hour"], "day": pl["day"], "policy": "local", "hide_client_headers": False},
    }
...
def build_tenant_config_plugin(tenant_id: str) -> dict:
    ...
    return {
        "apiVersion": "configuration.konghq.com/v1",
        "kind": "KongPlugin",
        "metadata": {"name": f"tenant-config-{tenant_id}", "namespace": ROMA_SYSTEM_NS},
        "plugin": "response-transformer",
        "config": {"add": {"headers": [f"X-Tenant-ID: {tenant_id}"]}},
    }
...
def build_service(tenant_id: str) -> dict:
    ...
    return {
        "apiVersion": "v1",
        "kind": "Service",
        "metadata": {"name": f"roma-{tenant_id}-api", "namespace": f"roma-{tenant_id}", "labels": {"roma.io/tenant": tenant_id}},
        "spec": {"type": "ClusterIP", "selector": {"roma.io/tenant": tenant_id}, "ports": [{"name": "http", "port": 8080, "targetPort": 8080}]},
    }
...
def ensure_kong_consumer(custom_api, tenant_id: str, api_key: str):
    try:
        custom_api.get_namespaced_custom_object(group="configuration.konghq.com", version="v1", namespace=KONG_NAMESPACE, plural="kongconsumers", name=tenant_id)
        logger.info(f"KongConsumer {tenant_id} already exists — skipping")
    except ApiException as e:
        if e.status == 404:
            custom_api.create_namespaced_custom_object(group="configuration.konghq.com", version="v1", namespace=KONG_NAMESPACE, plural="kongconsumers", body=build_kong_consumer(tenant_id, api_key))
...
def ensure_kong_plugin(custom_api, plugin_body: dict):
    ...
    try:
        custom_api.get_namespaced_custom_object(group="configuration.konghq.com", version="v1", namespace=ns, plural="kongplugins", name=name)
        logger.info(f"KongPlugin {name} already exists — skipping")
    except ApiException as e:
        if e.status == 404:
            custom_api.create_namespaced_custom_object(group="configuration.konghq.com", version="v1", namespace=ns, plural="kongplugins", body=plugin_body)
...
def reconcile_tenant(meta, spec, status, namespace, name, **kwargs):
    ...
    # Build labels
    labels = {"roma.io/plan": plan, "roma.io/tier": str(tier), "roma.io/tenant": tenant_id}
    ...
    try:
        custom.get_namespaced_custom_object("configuration.konghq.com", "v1", KONG_NAMESPACE, "kongcredentials", f"{tenant_id}-credential")
    except ApiException as e:
        if e.status == 404:
            custom.create_namespaced_custom_object("configuration.konghq.com", "v1", KONG_NAMESPACE, "kongcredentials", build_kong_credential(tenant_id, api_key))
    ...
    patch = {
        "status": {
            ...
            "conditions": [{"type": "Provisioned", "status": "True", "message": "All resources created", "lastTransitionTime": "2026-04-18T00:00:00Z"}],
        }
    }

    try:
        custom.patch_cluster_custom_object(group="roma.io", version="v1", plural="romatenants", name=name, body=patch)
