...
def seed():
    ...
    pe.ingest({"type": "RAFT_STATE_SNAPSHOT", "ts": now, "payload": {"leader": "node-0", "term": 5, "commit_index": 148, "nodes": {"node-0": "leader", "node-1": "follower", "node-2": "follower"}}})
    _pe_refresh = time.time()
...
# ── Helpers ──────────────────────────────────────────────────────
STATUS_COLOR = {"SUBMITTED": "#6c757d", "QUEUED": "#ffc107", "SCHEDULED": "#17a2b8", "RUNNING": "#0d6efd", "COMPLETED": "#28a745", "FAILED": "#dc3545"}
...
def make_gpu_bar(node: str, state: dict) -> go.Figure:
    ...
    bar = go.Figure(go.Bar(y=[node], x=[used], name="Used", marker_color="#e74c3c", orientation="h"))
    bar.add_trace(go.Bar(y=[node], x=[free], name="Free", marker_color="#2ecc71", orientation="h"))
    bar.update_layout(
        barmode="stack",
        height=80,
    ...
    return go.Figure(
        go.Indicator(
            ...
            gauge={
                "axis": {"range": [0, 1]},
                "bar": {"color": "#f39c12"},
                "steps": [{"range": [0, 0.3], "color": "#2ecc71"}, {"range": [0.3, 0.7], "color": "#f39c12"}, {"range": [0.7, 1], "color": "#e74c3c"}],
            },
            title={"text": "Last Job Cost"},
            domain={"x": [0, 1], "y": [0, 1]},
        )
    )
...
def make_timeline() -> go.Figure:
    ...
    fig.update_layout(title="Execution Timeline", template="plotly_dark", xaxis_title="Time (ms)", height=150, showlegend=False, margin=dict(l=100, r=20, t=30, b=30))
    return fig
...
app.layout = dbc.Container(
    [
        ...
        dbc.Row(
            [
                dbc.Col([dbc.Card([dbc.CardHeader("📊 Execution Timeline"), dbc.CardBody(dcc.Graph(id="timeline-chart", figure=make_timeline()))], className="mb-3")], width=8),
                dbc.Col([dbc.Card([dbc.CardHeader("💰 Last Job Cost"), dbc.CardBody(dcc.Graph(id="cost-gauge", figure=make_cost_gauge()))], className="mb-3")], width=4),
            ]
        ),
        # ── GPU Heatmap ──────────────────────────────────────────────
        dbc.Row([dbc.Col([dbc.Card([dbc.CardHeader("🎮 GPU Utilization Heatmap"), dbc.CardBody(dcc.Graph(id="gpu-chart"))], className="mb-3")], width=12)]),
        # ── Raft + Queue ────────────────────────────────────────────
        dbc.Row(
            [
                dbc.Col(
                    [
                        dbc.Card(
                            [dbc.CardHeader("🗳️ Raft Consensus State"), dbc.CardBody(html.Pre(id="raft-state", children="loading...", style={"font-size": "11px", "color": "#00ff41"}))],
                            className="mb-3",
                        )
                    ],
                    width=6,
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [dbc.CardHeader("📋 Job Queue"), dbc.CardBody(html.Ul(id="job-list", children=[html.Li(f"{j['job_id']} — {j['status']}") for j in pe.project_execution_timeline()]))],
                            className="mb-3",
                        )
                    ],
                    width=6,
        ...
        # ── Audit Log ────────────────────────────────────────────────
        dbc.Row([dbc.Col([dbc.Card([dbc.CardHeader("🔍 Immutable Audit Log"), dbc.CardBody(html.Pre(id="audit-log", children="", style={"font-size": "10px"}))], className="mb-3")], width=12)]),
        # ── Cost Simulator ──────────────────────────────────────────
        dbc.Row(
            [
                dbc.Col(
                            ...
                            [
                                ...
                                dbc.CardBody(
                                    [
                                        dbc.InputGroup(
                                            [
                                                dbc.InputGroupText("Task"),
                                                dbc.Input(id="sim-task", value="train YOLOv8"),
                                                dbc.InputGroupText("GPU sec"),
                                                dbc.Input(id="sim-gpu", value=60, type="number"),
                                            ],
                                            className="mb-2",
                                        ),
                                        dbc.InputGroup(
                                            [
                                                ...
                                                dbc.Input(id="sim-batch", value=32, type="number"),
                                                dbc.InputGroupText("Tier"),
                                                dbc.Select(
                                                    id="sim-tier",
                                                    options=[
                ...
                dbc.Col([dbc.Card([dbc.CardHeader("📈 System Health"), dbc.CardBody(html.Div(id="health-summary"))], className="mb-3")], width=6),
            ]
        ),
    ],
    fluid=True,
...
def refresh(n):
    ...
    jobs = pe.project_execution_timeline()
    job_items = [html.Li(html.Span(j["job_id"], style={"color": STATUS_COLOR.get(j["status"], "#fff")}), f" | {j['status']} | gpu={j['gpu']} | ${j['cost']:.4f}") for j in jobs]
...
def simulate(task, gpu_sec, batch, tier):
    ...
    if batch and batch > 64:
        base_cost *= 1.5
    return dbc.Alert([html.Strong(f"Estimated: ${base_cost:.4f}"), html.Br(), f"{tier} tier · {gpu_sec}s GPU · batch={batch}"], color="info")
