...
def seed():
    ...
    for i, ev in enumerate(
        [
            ...
            {
                "type": "JOB_SCHEDULED",
                "payload": {"job_id": "job-001", "gpu_node": "gpu-node-1"},
            },
...
def make_gpu_bar(node: str, state: dict) -> go.Figure:
    ...
    bar = go.Figure(
        go.Bar(y=[node], x=[used], name="Used", marker_color="#e74c3c", orientation="h")
    )
    bar.add_trace(
        go.Bar(y=[node], x=[free], name="Free", marker_color="#2ecc71", orientation="h")
    )
    bar.update_layout(
        barmode="stack",
        height=80,
...
app.layout = dbc.Container(
    [
        dbc.Row(
            dbc.Col(
                html.H1("🧠 ROMA — Observable OS", className="text-center my-3"),
                width=12,
            )
        ),
                    ...
                    [
                        dbc.Card(
                            [
                                ...
                                dbc.CardBody(
                                    dcc.Graph(
                                        id="timeline-chart", figure=make_timeline()
                                    )
                                ),
                            ],
                            className="mb-3",
                    ...
                    [
                        dbc.Card(
                            [
                                ...
                                dbc.CardBody(
                                    dcc.Graph(id="cost-gauge", figure=make_cost_gauge())
                                ),
                            ],
                            className="mb-3",
                        )
                                    ...
                                    [
                                        dbc.InputGroup(
                                            [
                                                ...
                                                dbc.Input(
                                                    id="sim-task", value="train YOLOv8"
                                                ),
...
def refresh(n):
    ...
    job_items = [
        html.Li(
            html.Span(
                j["job_id"], style={"color": STATUS_COLOR.get(j["status"], "#fff")}
            ),
    ...
    events = [
        f"[{e['ts']}] {e['type']} → {list(e.get('payload',{}).keys())}"
        for e in pe.events[-20:]
    ]
