...
if __name__ == "__main__":
    ...
    for i, (status, gpu, cost) in enumerate([("SUBMITTED", None, 0.0), ("QUEUED", None, 0.0), ("SCHEDULED", "gpu-node-1", 0.0), ("STARTED", "gpu-node-1", 0.0), ("COMPLETED", "gpu-node-1", 0.0275)]):
        pe.ingest({"type": f"JOB_{status}", "ts": now + i * 1000, "payload": {"job_id": "job-001", "tenant_id": "acme", "gpu_node": gpu, "cost": cost}})
    ...
    pe.ingest({"type": "RAFT_STATE_SNAPSHOT", "payload": {"leader": "node-0", "term": 5, "commit_index": 148, "nodes": {"node-0": "leader", "node-1": "follower", "node-2": "follower"}}})
