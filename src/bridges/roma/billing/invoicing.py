...
class InvoicingEngine:
    ...
    def generate(self, tenant_id: str, items: List[Dict], period_start: float, period_end: float) -> Invoice:
        subtotal = sum(i["cost"] for i in items)
        tax = subtotal * self.tax_rate
        inv = Invoice(
            invoice_id=f"INV-{uuid.uuid4().hex[:8].upper()}", tenant_id=tenant_id, period_start=period_start, period_end=period_end, line_items=items, subtotal=subtotal, tax=tax, total=subtotal + tax
        )
...
if __name__ == "__main__":
    ...
    inv = ie.generate(
        "tenant-abc", [{"desc": "GPU compute (5000s @ $0.00001)", "cost": 0.05}, {"desc": "Plugin executions (10x)", "cost": 0.01}], period_start=time.time() - 86400, period_end=time.time()
    )
