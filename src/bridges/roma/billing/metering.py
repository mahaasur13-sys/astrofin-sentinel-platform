...
class MeteringEngine:
    ...
    def record(self, event_type: str, tenant: str, job_id: str = "manual", gpu_seconds: float = 0, cpu_seconds: float = 0, gb_seconds: float = 0, plugin_count: int = 0) -> UsageEvent:
    ...
    def snapshot(self, tenant: str = None) -> Dict:
        ...
        return {"tenants": self.tenant_totals, "total_events": len(self.events), "total_cost": sum(t["cost"] for t in self.tenant_totals.values())}
...
if __name__ == "__main__":
    ...
    m.record("storage_usage", tenant="tenant-abc", gb_seconds=86400.0, job_id="roma-job-1")
