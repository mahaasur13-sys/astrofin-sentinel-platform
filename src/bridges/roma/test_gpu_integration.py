...
gate = scheduler.cost_gate.evaluate(
    task="ml_training", gpu_required=True, tenant_id="tenant_test", plugin_type="PRO"
)
print(
    f"  Cost gate: {gate.get('decision')}, quota: ${gate.get('quota_remaining', 0):.4f}"
)
...
print(
    f"  Local job route: {local_route['status']} → {local_route.get('execution_target')}"
)
...
print(
    f"  Rejected job: {rejected_route.get('status')} → reason: {rejected_route.get('reason', 'none')}"
)

# Test 5: Worker metrics
worker_metrics = connector.get_metrics()
print(
    f"  Worker metrics: {worker_metrics['available_workers']}/{worker_metrics['worker_count']} available"
)
