...
## KI-130 — Quality Gate misses orchestration/** and web/** paths

- **Status:** 🟢 RESOLVED
- **Severity:** 🟠 P1
- **Owner:** Felix (@mahaasur13-sys)
- **Reported:** 2026-07-13
- **Resolved:** 2026-07-13 (this commit)
- **Target version:** v1.1.0

### Problem

`.github/workflows/quality-gate.yml` listed only `agents/_impl/**`, `core/**`,
`scripts/**`, `docs/**`, `.github/workflows/**` under its `paths:` filter. Any
PR that touched `orchestration/**` or `web/**` did not trigger the Quality
Gate, so lint/type/coverage checks silently skipped those directories and
defects could land in master.

### Resolution

- Extended `paths:` with `orchestration/**` and `web/**` immediately after
  `core/**`.
- Re-verified that the Quality Gate job runs in `.github/workflows/quality-gate.yml`
  (file is present on `origin/master`, last touched in 399503d).
- Backfilled `DEPLOYMENT.md` line 221: `deploy/monitoring/prometheus/alerts.yml`
  → `deploy/monitoring/alerts.yml` (+ companion
  `deploy/monitoring/prometheus-alerts.yml` for the Prometheus ruleset) — the
  old path did not exist on disk.

### Acceptance

- PR touching `orchestration/**` triggers Quality Gate.
- PR touching `web/**` triggers Quality Gate.
- CI green on the PR that lands this change.

---

## KI-131 — OAuth token lacks `workflow` scope, blocking CI file edits

- **Status:** 🔴 NEW (active blocker)
- **Severity:** 🟠 P1
- **Owner:** Felix (@mahaasur13-sys)
- **Reported:** 2026-07-13
- **Target version:** v1.1.0
...
The default `gh` OAuth App token used by the agent's `git push` (account
`mahaasur13-sys`) does not carry the `workflow` OAuth scope. Any push that
touches `.github/workflows/**` is rejected by GitHub with:

```
remote: error: refusing to allow an OAuth App to create or update workflow
`.github/workflows/quality-gate.yml` without `workflow` scope
```

This blocks any change to CI workflow files — including the KI-130 fix —
from being pushed via the standard `gh`/`git` flow.

### Mitigation

A fine-grained/classic PAT with `gist, repo, workflow` scopes is stored in Zo
Secrets and exposed to the agent as the `AFS6` / `AFS7` environment
variables. The PAT is used to:

1. Clone the target branch with `x-access-token:` auth.
2. Apply the workflow + doc edits.
3. Push back to the branch.
4. Clean up the temporary clone and unset the token.

The PAT is **never** echoed to chat or written to files.

### Resolution (planned)

Promote the PAT to the canonical deployment credential, or rotate the
default `gh` OAuth token to include `workflow`. After that, KI-131 can be
closed and the temporary PAT-from-Secrets flow retired.

### Acceptance for un-parking

- `git push` of a `.github/workflows/**` change succeeds without manual PAT
  injection.
- KNOWN_ISSUES.md entry moved to RESOLVED.
