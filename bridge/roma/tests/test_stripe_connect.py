...
def test_fee_boundaries():
    ...
    for gross, expected_rate in cases:
        ...
        log.info(f"  [{'PASS' if passed else 'FAIL'}] ${gross:.2f} → {actual_rate*100:.0f}% (expected {expected_rate*100:.0f}%)")
        if not passed:
            all_passed = False
    return all_passed
...
def test_negative_amounts():
    ...
    try:
        ...
        log.info("  [FAIL] Negative amount was NOT rejected")
        return False
    except ValueError:
        log.info("  [PASS] Negative amount correctly rejected")
        return True
...
def test_zero_amount():
    ...
    if result['romas_share'] == 0 and result['partner_payout'] == 0:
        log.info("  [PASS] Zero amount → zero payout (both sides)")
        return True
    log.info("  [FAIL] Zero amount handling incorrect")
    return False
...
def test_application_fee():
    ...
    for cents, pct, expected_fee in cases:
        ...
        log.info(f"  [{'PASS' if passed else 'FAIL'}] {cents/100:.2f} @ {pct}% → fee={expected_fee/100:.2f}")
        if not passed:
            all_passed = False
    return all_passed
