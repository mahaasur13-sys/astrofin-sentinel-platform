...
class APIKeyManager:
    ...
    def create_key(self, org_id: str, project_id: str,
                   permissions: List[str], expires_in: int = 0) -> str:
        self._counter += 1
        key_id = f"kid_{org_id}_{project_id}_{self._counter}"
        raw = f"{PREFIX}{key_id}_{secrets.token_urlsafe(32)}"
        self._keys[key_id] = {
            "key_id": key_id, "org_id": org_id, "project_id": project_id,
            "permissions": permissions,
            "created": time.time(),
            "expires_at": time.time() + expires_in if expires_in else None,
            "revoked": False, "last_used": None, "rate_limit": 1000
        }
        return raw
    ...
    def validate_key(self, key: str) -> Optional[dict]:
        ...
        return {"valid": True, "org_id": stored["org_id"],
                "project_id": stored["project_id"], "permissions": stored["permissions"]}
