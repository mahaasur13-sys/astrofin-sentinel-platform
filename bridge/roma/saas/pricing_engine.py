...
TIER_QUOTA = {PricingTier.FREE: 3600, PricingTier.PRO: 180000, PricingTier.ENTERPRISE: 3600000}
...
class PricingEngine:
    def quote(self, gpu_seconds, gpu_model="RTX4090", tier="pro",
              region_mult=1.0, cluster_load=0.5):
        ...
        if tier_mult == 0:
            return PriceQuote(gpu_seconds=gpu_seconds, gpu_model=gpu_model, tier=tier,
                              base_cost=0.0, load_mult=lm, region_mult=region_mult,
                              total_cost=0.0, breakdown={"tier": tier, "quota_free": True,
        ...
        return PriceQuote(gpu_seconds=gpu_seconds, gpu_model=gpu_model, tier=tier,
                          base_cost=gpu_seconds*gpu_rate, load_mult=lm, region_mult=region_mult,
                          total_cost=total, breakdown={
                              "gpu_rate": gpu_rate, "tier_mult": tier_mult,
                              "region_mult": region_mult, "load_mult": lm,
                              "effective_rate": rate, "gpu_per_hour": rate*3600,
                              "monthly_quota_hrs": TIER_QUOTA.get(PricingTier(tier), 0)//3600,
...
class ProfitCalculator:
    ...
    def margin(self, quote: PriceQuote) -> Dict[str, Any]:
        ...
        return {"revenue": round(rev, 4), "cost": round(our_cost, 4),
                "profit": round(profit, 4), "GPM": round(gpm, 1),
