...
class StripeIntegration:
    ...
    def create_customer(self, org_id: str, email: str) -> StripeCustomer:
        c = StripeCustomer(id=f"cus_{uuid.uuid4().hex[:14]}", org_id=org_id, email=email,
    ...
    def create_subscription(self, org_id: str, tier: str) -> Subscription:
        now = time.time()
        s = Subscription(id=f"sub_{uuid.uuid4().hex[:12]}", org_id=org_id, tier=tier,
                        status=SubscriptionStatus.ACTIVE, current_period_start=now,
    ...
    def record_usage(self, org_id: str, key_id: str, gpu_seconds: float,
                    cost: float, job_id: str = "", plugin: str = "") -> UsageRecord:
        rec = UsageRecord(id=f"usr_{uuid.uuid4().hex[:12]}", org_id=org_id, key_id=key_id,
                         gpu_seconds=gpu_seconds, cost=cost, job_id=job_id, plugin=plugin)
    ...
    def get_usage_summary(self, org_id: str) -> Dict[str, Any]:
        ...
        return {"total_gpu_seconds": sum(r.gpu_seconds for r in records),
    ...
    def generate_invoice(self, org_id: str) -> Invoice:
        ...
        inv = Invoice(id=f"INV_{uuid.uuid4().hex[:8].upper()}", org_id=org_id,
                     items=[{"desc": r.plugin or "GPU compute", "gpu_s": r.gpu_seconds, "cost": r.cost}
                            for r in usage],
                     total=round(sum(r.cost for r in usage), 4), status="open")
