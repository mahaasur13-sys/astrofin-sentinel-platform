...
class PredictionResponse(BaseModel):
    ...
    explain_url: str | None = Field(None, description="URL to get SHAP explanations (if explain=true)")
...
class ExplainResponse(BaseModel):
    ...
    prediction_id: str
    risk_score: float
    shap_values: dict[str, float] = Field(..., description="SHAP values per feature — positive = pushes toward failure")
    feature_importance: dict[str, float] = Field(..., description="Absolute mean |SHAP| per feature")
    top_positive_features: list[str] = Field(..., description="Features most pushing toward failure")
    top_negative_features: list[str] = Field(..., description="Features most pushing away from failure")
