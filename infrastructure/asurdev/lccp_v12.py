        ...
        def h(s):
            return {
                k: (
                    ...
                    else ({kk: tuple(vv) for kk, vv in v.items()} if isinstance(v, dict) else v)
                )
                for k, v in s.items()
            }
...
def ctrl(issue):
    return {
        "NODE_FAILED": "RESTART",
        "DEGRADED_CPU": "SCALE_DOWN",
        "DEGRADED_MEMORY": "CLEAR_CACHE",
        "DEGRADED_STORAGE": "CLEANUP",
        "HEALTHY": "NO_ACTION",
        "OUT_OF_SCOPE": "REJECT",
    }.get(issue, "NO_ACTION")
...
def run_act(a, n):
    return {
        "SCALE_DOWN": f"scaled({n.id})",
        "CLEAR_CACHE": f"cache({n.id})",
        "CLEANUP": f"cleaned({n.id})",
        "NO_ACTION": "no_op",
        "RESTART": f"restart({n.id})",
        "REJECT": "rejected",
    }.get(a, "?")
