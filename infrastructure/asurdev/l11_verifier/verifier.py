...
class L11Verifier:
        ...
        for sr in sandbox_results:
            if not sr.allowed:
                failures.append(FailureReport(
                    FailureType.F4_RUNTIME_FAILURE, sr.node_id,
                    str(sr.violations), Action.ROLLBACK, recoverable=True
                ))
        return VerificationReport(
            stage="mid_execution",
            passed=len(failures) == 0,
    ...
    def post_execution(self, trace: dict, original_dag: dict | None = None) -> VerificationReport:
        ...
        if not valid:
            failures.append(FailureReport(
                FailureType.F3_NONDETERMINISM, None,
                "Hash chain verification failed", Action.INVALIDATE, recoverable=False
            ))
        return VerificationReport(
