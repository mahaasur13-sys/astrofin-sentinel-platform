...
try:
    ...
    from determinism_controller.controller import (
        DeterminismController,  # noqa: F401
        ExecutionContext,
    )
...
class L11Verifier:
    ...
    def pre_execution(
        self, dag: dict, context: dict | None = None
    ) -> VerificationReport:
        failures = []
        ...
        if not dag_result.valid:
            for v in dag_result.violations:
                ...
                failures.append(
                    FailureReport(ft, v.node_id, v.details, action, recoverable=False)
                )
    ...
    def post_execution(
        self, trace: dict, original_dag: dict | None = None
    ) -> VerificationReport:
        failures = []
    ...
    def full_pipeline(
        ...
        critical_failures = [
            f for f in all_failures if f.action in (Action.REJECT, Action.HARD_STOP)
        ]
        return {
            ...
            "action_needed": (
                critical_failures[0].action.value if critical_failures else "APPROVE"
            ),
