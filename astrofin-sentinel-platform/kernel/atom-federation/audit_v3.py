...
log.info("╔" + "═"*64 + "╗")
log.info("║  ATOMFederationOS v3.4 - FORMAL SYSTEM AUDIT          ║")
log.info("╚" + "═"*64 + "╝")
...
try:
    ...
    log.info(f"  Leaders count: {len(leaders)} (must be <=1)")
    log.info(f"  Node roles: {[(nid, getattr(dcp.nodes.get(nid), 'role', None)) for nid in state.get('nodes', {})]}")
    log.info(f"  {'✅' if ok_l1 else '❌'} L1 {'PASS' if ok_l1 else 'FAIL'}: Leader uniqueness verified")
except Exception as e:
    log.info(f"  ❌ L1 FAIL: {e}")
...
try:
    ...
    log.info(f"  5 identical updates -> unique states: {len(seen_states)}")
    log.info(f"  Event log entries: {len(bus._event_log)}")
    ok_l2 = len(bus._event_log) == 5
    log.info(f"  Event bus processed 5 events without error: {ok_l2}")
    log.info(f"  {'✅' if ok_l2 else '❌'} L2 {'PASS' if ok_l2 else 'FAIL'}: Event bus operational")
except Exception as e:
    log.info(f"  ❌ L2 FAIL: {e}")
...
try:
    ...
    log.info(f"  10 events -> last_hash: {h1[:16]}...")
    log.info(f"  After event+1 -> last_hash changed: {changed}")
    log.info(f"  Chain valid before: {chain_valid_before}, after: {chain_valid_after}")
    log.info(f"  {'✅' if ok_l3 else '❌'} L3 {'PASS' if ok_l3 else 'FAIL'}: Event store append-only verified")
except Exception as e:
    log.info(f"  ❌ L3 FAIL: {e}")
...
try:
    ...
    log.info(f"  file_read verdict: {v1} (is_veto={v1_is_veto})")
    log.info(f"  rm -rf verdict: {v2}")
    log.info(f"  Dangerous action blocked: {blocked_dangerous}")
    log.info(f"  {'✅' if ok_l4 else '❌'} L4 {'PASS' if ok_l4 else 'FAIL'}: Zero-trust (100%)")
except Exception as e:
    log.info(f"  ❌ L4 FAIL: {e}")
...
try:
    ...
    log.info(f"  20 tasks, Jain fairness: {fi:.3f}")
    ...
    log.info(f"  Queue heap-ordered (min-priority first): {is_min_heap}")
    ok_l5 = 0 < fi <= 1.0 and is_min_heap
    log.info(f"  {'✅' if ok_l5 else '❌'} L5 {'PASS' if ok_l5 else 'FAIL'}: Scheduler fair+correct")
except Exception as e:
    log.info(f"  ❌ L5 FAIL: {e}")

log.info("\n" + "═"*66)
all_ok = ok_l1 and ok_l2 and ok_l3 and ok_l4 and ok_l5
log.info(f"  OVERALL: {'✅ ALL TESTS PASSED' if all_ok else '❌ SOME TESTS FAILED'}")
log.info("═"*66)
