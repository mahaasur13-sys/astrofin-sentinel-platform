...
def mock_cluster_ctx():
    ...
    from cluster.node.health import ClusterHealthGraph
    health_graphs = {
...
class TestChaosHarness:
    def test_harness_runs_partition_scenario(self, mock_cluster_ctx):
        ...
        harness = ChaosHarness(scenario=scenario, cluster_ctx=mock_cluster_ctx, stabilization_time_s=2.0)
        result = harness.run()
    ...
    def test_harness_runs_all_scenarios(self, mock_cluster_ctx):
        results = []
        for name, scenario in SCENARIO_REGISTRY.items():
            harness = ChaosHarness(scenario=scenario, cluster_ctx=mock_cluster_ctx, stabilization_time_s=1.0)
            result = harness.run()
    ...
    def test_harness_collects_health_snapshots(self, mock_cluster_ctx):
        ...
        harness = ChaosHarness(scenario=scenario, cluster_ctx=mock_cluster_ctx, stabilization_time_s=1.0)
        result = harness.run()
        # ChaosResult doesn't expose health_snapshot; the run captured events
        assert isinstance(result.raw_events, list)
        assert len(result.raw_events) > 0
        # Sanity: at least one event has a stage recorded
        assert any(isinstance(ev, dict) and "stage" in ev for ev in result.raw_events)

    def test_run_scenario_convenience(self, mock_cluster_ctx):
        from chaos.scenarios import latency_spike
        scenario = latency_spike()
        harness = ChaosHarness(scenario=scenario, cluster_ctx=mock_cluster_ctx, stabilization_time_s=1.0)
        result = harness.run()
...
class TestIntegration:
    def test_node_isolation_complete(self, mock_cluster_ctx, validator):
        ...
        validation = validator.validate(
            ...
            health_states={nid: "unreachable" for nid in mock_cluster_ctx["nodes"]},
            sbs_results=[],
    ...
    def test_partition_half_cluster_verdict_not_fail(self, mock_cluster_ctx):
        ...
        result = harness.run()
        assert result.phase == ExperimentPhase.COMPLETE
        assert result.verdict != Verdict.FAIL, f"Cluster silently corrupted: {result}"
