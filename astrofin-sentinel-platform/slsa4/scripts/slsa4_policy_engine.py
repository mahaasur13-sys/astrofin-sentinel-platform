...
NATIVE_POLICY = {
    ...
    "g2": lambda d: d.get("manifest", {}).get("source") == "git-index"
    and d.get("manifest", {}).get("count", 0) > 0,
    "g3": lambda d: d.get("provenance", {}).get("verified")
    and d.get("provenance", {}).get("standard") == "slsa/in-toto",
    "g4": lambda d: d.get("lccp", {}).get("deterministic")
    and d.get("lccp", {}).get("replay_verified"),
    "g5": lambda d: d.get("cvg", {}).get("policy_valid")
    and d.get("cvg", {}).get("cross_repo_consistent"),
    "g6": lambda d: d.get("external", {}).get("verified")
    and d.get("external", {}).get("method") != "none",
}
...
class SLSA4PolicyEngine:
    def __init__(self, bundle_path=None):
        self.bundle_path = bundle_path or os.environ.get(
            "SLSA4_BUNDLE", "/tmp/attestation_bundle.json"
        )
    ...
    def evaluate_native(self, bundle: dict) -> dict:
        ...
        for gate in sorted(gate_names):
            ...
            results[f'G{gate[-1] if gate.startswith("g") else gate}'] = {
                "passed": passed
            }
