...
def generate_in_toto_layout(
    links: list, functionaries: list, inspection: list = None
) -> dict:
...
def slsa_v1_provenance(
    commit: str, builder_id: str, materials: list, invocation: dict
) -> dict:
    ...
    predicate = {
        ...
        "materials": materials,
        "metadata": {
            "buildStartedOn": datetime.datetime.now(datetime.timezone.utc).isoformat()
            + "Z",
            "buildFinishedOn": datetime.datetime.now(datetime.timezone.utc).isoformat()
            + "Z",
            "reproducible": True,
    ...
    stmt = {
        ...
        "subject": [
            {"name": f"git-commit/{commit[:12]}", "digest": {"sha256": commit}}
        ],
        "predicate": predicate,
    }
...
def generate_supply_chain_attestation(
    repo: str, commit: str, manifest_hash: str, lccp_result: dict
) -> dict:
    ...
    products = [
        {
            "digest": {"sha256": h(commit + manifest_hash)},
            "uri": f"git:/{commit}/artifacts",
        },
    ]
    links = [
        generate_in_toto_link(
            "g0_setup", {}, {}, ["python3", "build_cvg.py"], {"PYTHON_VERSION": "3.12"}
        ),
        generate_in_toto_link(
            "g1_build",
            {m["uri"]: m["digest"] for m in materials},
            {},
            ["make", "bootstrap"],
            {"HERMETIC": "true"},
        ),
        ...
        generate_in_toto_link(
            "g3_provenance", {}, products, ["python3", "slsa4_policy_engine.py"], {}
        ),
        generate_in_toto_link(
            "g4_lccp",
            materials,
        ...
        generate_in_toto_link(
            "g5_cvg", materials, {}, ["python3", "build_cvg.py", "--verify"], {}
        ),
        generate_in_toto_link(
    ...
    attestation = {
        ...
        "supply_chain_links": [
            {"name": l["name"], "link_hash": l["link_hash"]} for l in links
        ],
        "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(),
        "slsa_level": 4,
        "release_allowed": True,
    }
    attestation["bundle_hash"] = h(
        json.dumps(
            {k: v for k, v in attestation.items() if k != "bundle_hash"},
            sort_keys=True,
            default=str,
        )
    )
...
if __name__ == "__main__":
    commit = (
        sys.argv[1]
        if len(sys.argv) > 1
        else h(datetime.datetime.now().isoformat())[:12]
    )
