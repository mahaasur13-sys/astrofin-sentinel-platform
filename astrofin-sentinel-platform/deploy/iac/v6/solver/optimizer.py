...
class HardConstraintPruner:
    ...
    def prune(
        self, job_id: str, candidates: list[tuple[str, float]]
    ) -> list[tuple[str, float]]:
        """
...
class ILPOptimizer:
    ...
    def solve(
        ...
        try:
            ...
            # Objective
            prob += pulp.lpSum(
                x[j, n] * utilities.get((j, n), 0)
                for j in job_ids
                for n in candidates.get(j, [])
            )
            ...
            for node in all_nodes:
                prob += (
                    pulp.lpSum(x[(job, node)] for job in job_ids if (job, node) in x)
                    <= 1
                )
...
class HybridSolver:
    ...
    def solve(
        ...
        if self.config.enable_ilp and len(valid) <= self.config.k_candidates:
            assignment = self.ilp.solve(
                [job_id], {job_id: [n for n, _ in valid]}, utilities
            )
