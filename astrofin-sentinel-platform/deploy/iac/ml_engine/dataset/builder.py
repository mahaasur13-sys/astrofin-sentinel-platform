...
class DatasetBuilder:
    ...
    def build(
        ...
        features_df["time_bucket"] = features_df["time"].dt.floor(
            f"{window_type.replace('m', 'min')}"
        )

        labels_df = labels_df.copy()
        labels_df["label_bucket"] = labels_df["failure_time"].dt.floor(
            f"{window_type.replace('m', 'min')}"
        )
        ...
        df["label_gpu_util"] = df["label_gpu_util"].fillna(
            df.get("gpu_util_5m_avg", 0.0)
        )
    ...
    def _label_from_timescale(
        ...
        try:
            labels_df = pd.read_sql(
                query, conn, params={"start": start_time, "end": end_time}
            )
