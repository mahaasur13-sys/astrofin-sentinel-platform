...
def evaluate_regressor(
    model, X_test, y_queue_test, y_gpu_test=None
) -> dict[str, float]:
    ...
    if y_gpu_test is not None:
        metrics["gpu_mae"] = float(
            mean_absolute_error(y_gpu_test, model.predict_gpu(X_test))
        )
        metrics["gpu_rmse"] = float(
            np.sqrt(mean_squared_error(y_gpu_test, model.predict_gpu(X_test)))
        )
