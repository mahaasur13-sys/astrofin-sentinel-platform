...
class Trainer:
    ...
    def train(
        ...
        if use_smote:
            ...
            logger.info(
                f"SMOTE applied — before: {len(X_train)}, after: {len(X_train_bal)}"
            )
        else:
        ...
        if output_dir:
            ...
            version_id = self.registry.register(
                ...
                params=(
                    failure_model.get_params()
                    if hasattr(failure_model, "get_params")
                    else {}
                ),
        ...
        load_metrics = evaluate_regressor(
            load_model, X_test, yq_test, y_gpu_test=yg_test
        )
        logger.info(
            f"Load model — Queue MAE: {load_metrics['queue_mae']:.4f}, GPU MAE: {load_metrics['gpu_mae']:.4f}"
        )
