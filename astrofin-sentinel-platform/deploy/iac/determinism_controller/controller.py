...
class DeterminismController:
    ...
    def verify_replay(
        self, original_trace: dict, replay_trace: dict
    ) -> tuple[bool, list[str]]:
        errors = []
        for key in original_trace:
            ...
            if original_trace[key] != replay_trace[key]:
                errors.append(
                    f"Field '{key}': original={original_trace[key]}, replay={replay_trace[key]}"
                )
    ...
    def get_determinism_report(self, trace_a: dict, trace_b: dict) -> dict[str, Any]:
        ...
        return {
            ...
            "r2_scheduler": match,
            "r3_ml_locked": all(
                v in self.ctx.pinned_versions for v in self.ctx.ml_model_snapshot
            ),
