...
class DeterministicReplayer:
    ...
    def replay(
        self, trace_id: str, hooks: dict[str, Callable] | None = None
    ) -> ReplayResult:
        trace = self.store.get_trace(trace_id)
        if not trace:
            raise ValueError(f"Trace {trace_id} not found")
...
class CorrelationEngine:
    ...
    def find_divergence(
        self, run_id: str, baseline_run_id: str, layer: str
    ) -> list[dict[str, Any]]:
        baseline_nodes = {
            n["node_id"]: n for n in self.query_by_layer(baseline_run_id, layer)
        }
        current_nodes = {n["node_id"]: n for n in self.query_by_layer(run_id, layer)}
        divergences = []
