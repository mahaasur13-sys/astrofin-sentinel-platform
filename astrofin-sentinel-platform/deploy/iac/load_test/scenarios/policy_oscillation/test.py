...
class PolicyOscillationScenario:
        ...
        while time.time() - start < duration_sec:
            ...
            if self.job_count % self.burst_cycle == 0:
                self.current_workload = (
                    "idle" if self.current_workload == "burst" else "burst"
                )
    ...
    def _make_decision(self) -> PolicyState:
        ...
        decision = (
            "accept"
            if final_score > 0.5
            else "defer" if final_score > 0.3 else "reject"
        )
    ...
    def _compute_metrics(self) -> dict:
        ...
        if self.switches:
            time_span = (
                self.switches[-1] - self.switches[0] if len(self.switches) > 1 else 1
            )
    ...
    def _simulate_after_fix(self, fix: str) -> dict:
        ...
        return {
            ...
            "improvement": (
                "stabilized" if new_switches < self.total_switches else "not_improved"
            ),
        }
