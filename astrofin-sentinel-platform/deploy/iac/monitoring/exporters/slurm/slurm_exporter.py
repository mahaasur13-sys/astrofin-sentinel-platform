...
def get_slurm_queue() -> dict:
    ...
    re.compile(
        r"^(.+?)\s+(.+?)\s+(COMPLETING|RUNNING|PENDING|FAILED|CANCELLED)\s+(.*)$"
    )
...
def get_slurm_nodes() -> dict:
    ...
    try:
        out = subprocess.check_output(
            ["sinfo", "-N", "--format=%N|%A|%a|%c|%m|%e|%G|%T"], text=True, timeout=5
        )
    except Exception:
        return {}
...
def build_metrics() -> str:
    ...
    for name, info in nodes.items():
        labels = f'node="{name}"'
        lines.append(
            f"slurm_node_available{{{labels}}} {1 if info['available'] else 0}"
        )
        lines.append(f"slurm_node_gpus{{{labels}}} {info['gpus']}")
        lines.append(
            f"slurm_node_state{{{labels}}} 1"
            if info["state"] == "alloc"
            else f"slurm_node_state{{{labels}}} 0"
        )
