...
class DriftAlignment:
    ...
    def record_signals(self, signals: DriftSignals) -> None:
        ...
        self._model_error_history = [
            m for m, mk in zip(self._model_error_history, mask) if mk
        ]
        self._system_drift_history = [
            s for s, m in zip(self._system_drift_history, mask) if m
        ]
    ...
    def compute_alignment(self) -> AlignmentResult:
        ...
        if alignment < self.retrain_threshold:
            regime = "retrain"
            recommendation = (
                "critical drift misalignment — retrain v5 model immediately"
            )
