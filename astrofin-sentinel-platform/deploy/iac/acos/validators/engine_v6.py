...
class EventSourcedEngine:
    ...
    def __init__(
        self, event_log, trace_recorder: DeterministicTraceRecorder | None = None
    ):
        self._log = event_log
    ...
    def execute(self, dag: dict, context: dict, trace_id: str) -> str:
        ...
        if violations:
            raise ValueError(
                f"DAG validation failed: {[v.message for v in violations]}"
            )
        ...
        self._log.emit(
            trace_id, "DAG_CREATED", {"dag": dag, "context": context, "started_at": t0}
        )
        ...
        for node in dag.get("nodes", []):
            ...
            self._log.emit(
                trace_id, "NODE_EXECUTED", {"node_id": node_id, "result": "success"}
            )

        # Phase 5: Complete
        self._log.emit(
            trace_id, "TRACE_RECORDED", {"final_state": {"status": "COMPLETED"}}
        )
