...
class TunnelEvent:
    ...
    def _compute_hash(self) -> str:
        ...
        data = (
            f"{self.trace_id}{self.event_type}{self.timestamp}"
            ""
            f"{''}{self.prev_hash}"
        )
...
class AmneziaWGManager:
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    for line in proc.stdout.splitlines():
                        if "transfer:" in line:
                            result["transfer_bytes"] = line.split("transfer:")[
                                1
                            ].strip()
                    break
            except Exception:
    ...
    def reconnect_with_backoff(self, attempt: int = 0) -> bool:
        ...
        logger.info(
            f"[{self._iface}] reconnect attempt {attempt+1}, delay={delay:.2f}s (deterministic)"
        )
        time.sleep(delay)
        self._emit(
            "TUNNEL_FAILOVER", f"Reconnecting, attempt={attempt+1}, delay={delay:.2f}s"
        )
        self._started = False
        ok = self.start()
        if ok:
