...
class DAGValidator:
    ...
    def validate_dag(dag: dict[str, Any]) -> list[ContractViolation]:
        ...
        for node in nodes:
            ...
            if not node_id:
                violations.append(
                    ContractViolation("Node missing 'id'", "/nodes", "error")
                )
            elif node_id in node_ids:
                violations.append(
                    ContractViolation(
                        f"Duplicate node id: {node_id}", f"/nodes/{node_id}", "error"
                    )
                )
            else:
        ...
        for edge in edges:
            ...
            if src not in node_ids:
                violations.append(
                    ContractViolation(
                        f"Edge source '{src}' not found", f"/edges/{edge}", "error"
                    )
                )
            if tgt not in node_ids:
                violations.append(
                    ContractViolation(
                        f"Edge target '{tgt}' not found", f"/edges/{edge}", "error"
                    )
                )
        ...
        if trace_id:
            try:
                ...
                violations.append(
                    ContractViolation(
                        f"Invalid trace_id format: {trace_id}", "/dag/trace_id", "error"
                    )
                )
    ...
    def validate_event(event: dict[str, Any]) -> list[ContractViolation]:
        ...
        for field in required_fields:
            if field not in event:
                violations.append(
                    ContractViolation(
                        f"Missing field '{field}' in event", "/event", "error"
                    )
                )
        return violations
    ...
    def validate_trace(trace_data: dict[str, Any]) -> list[ContractViolation]:
        ...
        if "trace_id" not in trace_data:
            violations.append(
                ContractViolation("Missing trace_id in trace", "/trace", "error")
            )
        if "events" not in trace_data or not isinstance(trace_data["events"], list):
            violations.append(
                ContractViolation("Missing or invalid 'events' list", "/trace", "error")
            )
        return violations
