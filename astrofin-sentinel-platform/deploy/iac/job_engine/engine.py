...
class JobEventHooks:
    ...
    def on_submit(self, job: Job) -> None:
        logger.info(
            f"[{job.id}] submitted (type={job.job_type}, priority={job.priority})"
        )
...
class TelemetryJobEngine(JobEventHooks):
    ...
    def admit(self, job_id: str, node_id: str) -> bool:
        ...
        return self._transition(
            job, JobState.ADMITTED, extra=lambda: setattr(job, "node_id", node_id)
        )
    ...
    # ---------------------------------------------------------------------------
    def _transition(
        self, job: Job, new_state: JobState, extra: Callable = None
    ) -> bool:
        old_state = job.state
        if new_state not in TRANSITIONS.get(old_state, []):
            logger.warning(
                f"[{job.id}] INVALID TRANSITION {old_state.value} → {new_state.value}"
            )
            return False
        with self._lock:
            job.state = new_state
    ...
    def _write_event(
        self, job: Job, state: JobState, error: str | None, event_type: str
    ) -> None:
