...
class ACOSOrchestrator:
    ...
    def decision(self, request: ACOSDecisionRequest) -> ACOSDecisionResponse:
        trace = self.trace_store.create_trace(
            request.context.run_id if request.context else "default"
        )
        ...
        if self.execution_gate:
            gate_decision = self.execution_gate.check(
                ctx, request.action, request.params
            )
...
if __name__ == "__main__":
    ...
    print(
        f"ACOS v3.0 — Confidence: {result['CONFidence']['final']} — Decision: {result['DECISION']['action']}"
    )
