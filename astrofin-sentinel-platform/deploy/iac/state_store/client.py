...
class StateStore:
    ...
    def __init__(
        self, host="localhost", port=5432, dbname="clusterdb", user=None, password=None
    ):
        self.host = host
        self.port = port
        self.dbname = dbname
    ...
    def create_job(
        ...
        self._write_event(
            job_id, "JOB_CREATED", {"priority": priority, "memory_gb": memory_gb}
        )
        return job_id
    ...
    def write_scheduler_decision(
        self, job_id: str, round_num: int, scores: list[dict], selected_node: str
    ) -> None:
        with self._cursor() as cur:
    ...
    def write_admission_decision(
        self, job_id: str, decision: str, reason: str, cluster_util: dict
    ) -> None:
        with self._cursor() as cur:
            cur.execute(
