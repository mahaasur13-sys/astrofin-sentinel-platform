...
FEATURE_SPECS: list[FeatureSpec] = [
    ...
    FeatureSpec(
        ...
        source="failure_events",
        window_seconds=86400,
        aggregation="last_age_min",
        unit="min",
        typical_range=(0, 1440),
    ),
    FeatureSpec(
    ...
    FeatureSpec(
        ...
        source="gpu_util",
        window_seconds=300,
        aggregation="overload_composite",
        unit="score",
        typical_range=(0, 100),
    ),
    FeatureSpec(
        ...
        source="cpu_util",
        window_seconds=300,
        aggregation="health_composite",
        unit="score",
        typical_range=(0, 100),
    ),
    FeatureSpec(
        name="queue_volatility_5m",
        source="queue_size",
        window_seconds=300,
...
def validate_feature_vector(fv: FeatureVector) -> list[str]:
    ...
    for spec in FEATURE_SPECS:
        if spec.name in fv.features:
            ...
            if val < lo or val > hi:
                warnings.append(
                    f"{spec.name}={val:.2f} outside typical range [{lo},{hi}]"
                )
    return warnings
