...
DEFAULT_WINDOWS = [
    WindowConfig(
        name="1m", size_seconds=60, aggregates=["mean", "std", "min", "max", "latest"]
    ),
    WindowConfig(
        name="5m",
        size_seconds=300,
    ...
    WindowConfig(
        name="15m", size_seconds=900, aggregates=["mean", "std", "max", "slope"]
    ),
...
class WindowEngine:
    ...
    def _get_or_create_window(
        self, node_id: str, metric: str, window_seconds: int
    ) -> SlidingWindow:
    ...
    def push(
        self, node_id: str, metric: str, value: float, timestamp: datetime | None = None
    ) -> None:
    ...
    def get_aggregated(
        self, node_id: str, metric: str, window_seconds: int, agg: str
    ) -> float:
        key = (node_id, metric, window_seconds)
        if key not in self._storage:
            return 0.0
