...
def parse_args():
    ...
    p.add_argument(
        "--validate-registry", action="store_true", help="Validate feature registry"
    )
    p.add_argument("--continuous", action="store_true", help="Run continuous pipeline")
    p.add_argument(
        "--interval", type=int, default=60, help="Sampling interval in seconds"
    )
    p.add_argument(
        "--nodes",
        type=str,
    ...
    p.add_argument(
        "--export-parquet", action="store_true", help="Export Parquet dataset"
    )
    p.add_argument(
        "--output",
        type=str,
    ...
    p.add_argument(
        "--horizon", type=int, default=30, help="Prediction horizon in minutes"
    )
    p.add_argument(
        "--embedding", action="store_true", help="Compute and show node embeddings"
    )
    p.add_argument(
...
def main():
    ...
    if args.validate_registry:
        log.info(
            "Validating feature registry (%d features)...", len(get_feature_names())
        )
        validate_registry()
