...
def gate(
    lr: float, loss: float, state: CycleState, verbose: bool = False
) -> tuple[bool, str]:
...
def run_cycle(
    verbose: bool = False, max_iterations: int = DEFAULT_MAX_ITERATIONS
) -> CycleState:
...
def main() -> int:
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="подробный лог каждой итерации"
    )
    parser.add_argument(
        "--reset", action="store_true", help="сбросить state-файл перед запуском"
    )
    parser.add_argument(
        "--iterations",
        type=int,
