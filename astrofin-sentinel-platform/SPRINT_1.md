...
> **Source:** `PRODUCTION_BACKLOG.md` (Top-15 W1) + критический путь
...
|---|----|--------|-------|-----:|------|------------|--------------------|
| 1 | **P0-01** | Расследовать 26 failing tests: `pytest --collect-only`, классифицировать в `tests/FAILING_TESTS.md` | Backend | 3 | 09:00-12:00 | — | Файл создан, ≥5 зафиксировано как flaky/import, остальные перенесены в issue |
...
| 5 | **P0-06** | Прочитать 6 bandit high, классифицировать | Security | 2 | 10:00-12:00 | — | `docs/security/bandit-baseline.md` создан |

**Daily target:** 9 ч × 4 треков = 36 ч. **Subtotal Mon: 9 ч**

**Standup (17:00):** показать `FAILING_TESTS.md` + `.gitignore` PR + branch protection.
...
| # | ID | Задача | Owner | Часы | Время | Зависит от | Definition of Done |
|---|----|--------|-------|-----:|------|------------|--------------------|
| 6 | **P0-03** | План submodule→subtree: написать `docs/MIGRATION_submodules.md` | DevOps | 4 | 09:00-13:00 | — | Документ: dry-run команды, rollback, contact list |
...
| 8 | **P0-05** | Сгенерировать `requirements.lock` через `pip-compile` | Backend | 2 | 09:00-11:00 | — | `requirements.lock` коммитится, `python-setup.yml` его использует |
| 9 | **P0-07** | ADR-0001: «Adopt 13-agent hybrid signal architecture» | Writer | 2 | 11:00-13:00 | — | `docs/adr/0001-hybrid-agents.md` в шаблоне MADR |
| 10 | **P0-01.c** | Починить ещё 2 broken tests (async timeout в `tests/test_council.py`) | Backend | 3 | 14:00-17:00 | #2 | pytest count: 26 → 19 fail |
...
| 12 | **P1-01.b** | `tools/check_env.py` — fail-fast на отсутствующие ключи | DevOps | 1 | 11:00-12:00 | #11 | `python tools/check_env.py --prod` exit 1 без env, exit 0 с шаблоном |
| 13 | **P1-02** | SOPS+age интеграция: установить sops в CI, создать `.sops.yaml` | DevOps | 4 | 13:00-17:00 | #11 | `sops --decrypt .env.prod.enc` работает локально и в CI |
| 14 | **P1-02.b** | k8s init-container скрипт: `tools/secrets-init.sh` (decrypts SOPS на старте) | DevOps | 1 | 17:00-18:00 | #13 | Скрипт работает в `kind` локально, output в `/run/secrets/.env` |
| 15 | **P1-03** | `core/auth/jwt.py`: RS256, `/auth/login`, `/auth/refresh`, JWKS endpoint | Backend | 4 | 09:00-13:00 | — | `pytest tests/test_jwt.py` зелёный, `/auth/login` выдаёт tokens |
...
| 17 | **P3-01** | SLO/SLI черновик: latency, error rate, availability для 3 API | Writer | 2 | 14:00-16:00 | — | `docs/SLO.md` черновик с 3 SLI определениями + error budget формулой |
...
| # | ID | Задача | Owner | Часы | Время | Зависит от | Definition of Done |
|---|----|--------|-------|-----:|------|------------|--------------------|
| 25 | **P1-06** | CORS whitelist (не `*`): `ALLOWED_ORIGINS` env, применяется в `web/app.py` + FastAPI middleware | Backend | 1 | 09:00-10:00 | — | `curl -H "Origin: evil.com"` → нет `Access-Control-Allow-Origin` |
...
| 30 | **Sprint Review** | Подготовка demo для команды: 30 мин скринкаст, summary | Backend + Writer | 1 | 17:00-18:00 | All W1 | Записано в `docs/sprints/2026-W27_REVIEW.md` |
...
- [ ] `docs/SLO.md` согласован командой
- [ ] `docs/MIGRATION_submodules.md` существует, dry-run проверен
- [ ] `docs/adr/0001-hybrid-agents.md` в формате MADR
...
Создать `docs/sprints/2026-W27_BURNDOWN.md` с ежедневным обновлением:
...
| W1-R2 | **JWT-библиотека конфликтует с существующей `core/auth.py`** | План Б: P1-03 dual-mode (API_KEY + JWT) 2 недели, потом deprecate API_KEY |
...
- **Daily update:** в конце дня обновить burndown chart, написать 3-5 строк в `docs/sprints/2026-W27_DAILY_<date>.md`
