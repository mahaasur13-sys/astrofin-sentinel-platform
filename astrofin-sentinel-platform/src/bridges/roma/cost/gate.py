...
class DecisionGate:
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if not task or not isinstance(task, str):
            raise ValueError(
                "Параметр 'task' обязателен и должен быть непустой строкой."
            )
        ...
        try:
            ...
            if decision not in ("APPROVED", "REQUIRES_CONFIRMATION", "REJECTED"):
                ...
                if cost > limit:
                    ...
                    cost == 0.0 or cost > limit * 0.7
                ):  # $0.00 считаем как "требует подтверждения"
    ...
    def _create_error_response(
        self, task: str, tenant_id: str, exc: Exception
    ) -> GateResponse:
        return {
            "task": task,
            "tenant_id": tenant_id,
    ...
    def _gate_message(
        self, decision: str, cost: float, tier: str, risk_flags: List[str]
    ) -> str:
        limit = self.TIER_LIMITS.get(tier, self.DEFAULT_LIMIT)
    ...
    def get_gate_status(self, tenant_id: str = "default-tenant") -> Dict[str, Any]:
        ...
        tenant_decisions = [
            d for d in self.decision_history if d.get("tenant_id") == tenant_id
        ]
        ...
        return {
            ...
            "approved": sum(
                1 for d in tenant_decisions if d.get("decision") == "APPROVED"
            ),
            "rejected": sum(
                1 for d in tenant_decisions if d.get("decision") == "REJECTED"
            ),
            "requires_confirmation": sum(
                1
                for d in tenant_decisions
                if d.get("decision") == "REQUIRES_CONFIRMATION"
            ),
        }
