...
class IPlugin(ABC):
    ...
    async def on_execute(
        self, task: "ROMATask", context: "ExecutionContext"
    ) -> "PluginResult": ...
...
class PluginResult:
    ...
    def __init__(
        self, success: bool, output: Any = None, error: str = None, metrics: Dict = None
    ):
...
class MLTrainingPlugin(IPlugin):
    ...
    async def on_execute(
        self, task: ROMATask, context: IExecutionContext
    ) -> PluginResult:
...
class ETLPipelinePlugin(IPlugin):
    ...
    async def on_execute(
        ...
        return PluginResult(
            success=True, output={"pipeline_id": task.task_id, "stage": "completed"}
        )
