...
class BillingLedger:
    ...
    def record_revenue_share(
        self, tenant_id: str, amount_cents: int, source_invoice: str, rate: float
    ) -> None:
        now = int(time.time())
...
def simulate_ledger() -> None:
    ...
    ledger.credit(
        "tenant-abc", 100.0, metadata={"source": "subscription", "plan": "PRO"}
    )
    ledger.credit("tenant-abc", 10.0, metadata={"source": "credit_topup"})
    ledger.debit(
        "tenant-abc", 3.20, metadata={"description": "GPU usage 2h @ $0.0016/sec"}
    )
    ledger.debit(
        "tenant-abc", 0.60, metadata={"description": "plugin execution 1 unit"}
    )
