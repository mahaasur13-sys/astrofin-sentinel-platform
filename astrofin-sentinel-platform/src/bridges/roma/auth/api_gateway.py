...
class RateLimiter:
    ...
    def check(self, tenant_id: str, rate_limit: int, window: int) -> Tuple[bool, dict]:
        ...
        return allowed, {
            ...
            "reset_in_sec": (
                round((1 - b["tokens"]) * window / rate_limit, 1)
                if b["tokens"] < 1
                else 0
            ),
        }
...
class APIGateway:
    ...
    def extract_key(self, request: dict) -> Optional[str]:
        ...
        return request.get("query", {}).get("api_key") or request.get(
            "headers", {}
        ).get("x-api-key")
    ...
    def middleware(self, func):
        ...
        def wrapper(request: dict):
            ...
            if not secret:
                return {
                    "error": {"code": "API_KEY_REQUIRED", "message": "API key missing"},
                    "status": 401,
                }
            ...
            if not allowed:
                return {
                    ...
                    "status": 429,
                    "headers": self._rate_headers(rl_info),
                }
            ...
            if quota_used >= quota_limit:
                return {
                    ...
                    "status": 402,
                    "headers": self._quota_headers(quota_used, quota_limit),
                }
...
if __name__ == "__main__":
    ...
    kid, sec = auth.create_key(
        "tenant-test", "p1", KeyType.SERVER, "test", scopes=["submit"]
    )
    ...
    print(
        f"Valid key → status={resp.get('status', 200)}, headers={list(resp.get('headers', {}).keys())}"
    )
