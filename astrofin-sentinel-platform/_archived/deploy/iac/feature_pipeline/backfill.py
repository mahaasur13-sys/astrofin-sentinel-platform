...
def backfill_range(
    ...
    node_filter = (
        f"AND node_id IN ({','.join(repr(n) for n in nodes)})" if nodes else ""
    )
    ...
    rows = cur.fetchall()
    logger.info(
        "retrieved_rows", count=len(rows), start=start.isoformat(), end=end.isoformat()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", default=os.path.join(tempfile.gettempdir(), "ml_dataset")
    )
