...
class PostgresTraceStorage:
    ...
    def fetch(self, trace_id: str) -> dict | None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT trace FROM acos_traces WHERE trace_id = %s", (trace_id,)
            )
            row = cur.fetchone()
    ...
    def query(self, filters: dict | None = None) -> list[dict]:
        ...
        with conn.cursor() as cur:
            if not filters:
                cur.execute(
                    "SELECT trace FROM acos_traces ORDER BY created_at DESC LIMIT 100"
                )
            else:
                # Simple key-value filter
                where = " AND ".join("trace->>%s = %s" for _ in filters)
                cur.execute(
