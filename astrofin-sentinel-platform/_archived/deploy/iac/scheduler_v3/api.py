...
SCHEDULER_REQUESTS = Counter(
    "scheduler_requests_total", "Total schedule requests", ["job_type", "decision"]
)
    ...
    try:
        ...
        if not best_node:
            ...
            return ScheduleResponse(
                submitted=False, job_id=job_id, reason="no healthy node available"
            )

        # Step 3: Record scores for determinism testing
        get_store().write_scheduler_decision(
            job_id, round_num=1, scores=all_scores, selected_node=best_node.hostname
        )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        app, host=os.environ.get("BIND_HOST", "127.0.0.1"), port=8080, log_level="info"
    )
