...
check_reqs() {
  for bin in ansible ansible-playbook terraform git curl; do
    command -v $bin &>/dev/null || err "Required binary missing: $bin"
  done
  # docker is optional — only required for ml-api subcommand
  if ! command -v docker &>/dev/null; then
    warn "docker not found — ml-api subcommand will be unavailable"
  fi
