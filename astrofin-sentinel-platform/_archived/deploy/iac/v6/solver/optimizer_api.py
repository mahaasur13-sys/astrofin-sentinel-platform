...
def optimize(req: OptimizationRequest) -> OptimizationResult:
        ...
        result = ilp._fallback_heuristic(
            [
                type(
                    "C",
                    (),
                    {"job_id": c.job_id, "node_id": c.node_id, "score": c.score},
                )()
                for c in candidates
            ],
    ...
    twin_actions = [
        {
            "action_type": "place_job",
            "job_id": p["job_id"],
            "target_node": p["node_id"],
            "job_config": next(
                (j for j in req.pending_jobs if j.get("id") == p["job_id"]), {}
            ),
        }
    ...
    _get_policy()
    best_idx = max(
        range(len(twin_results)),
        key=lambda i: twin_results[i].get("expected_utility", 0),
    )
    final_placements = (
        [result.placements[best_idx]] if twin_results else result.placements[:1]
    )
    ...
    init_state = SimState(
        ...
        nodes={
            nid: NodeState(
                ...
                cpu_util_pct=0.3,
                cpu_mem_used_gb=8.0,
                cpu_mem_total_gb=32.0,
                gpu_temp_c=45.0,
                failure_prob_30m=0.1,
                load_forecast_15m=0.3,
                active_jobs=1,
                wireguard_peers_up=2,
