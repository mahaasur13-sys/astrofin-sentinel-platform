...
class ReplayEngine:
    ...
    def _compare_traces(self, original: dict, replayed: dict) -> bool:
        if original["final_state"] != replayed["final_state"]:
            return False
        orig_nodes = {
            n["node_id"]: n["status"] for n in original.get("node_execution_log", [])
        }
        repl_nodes = {
            n["node_id"]: n["status"] for n in replayed.get("node_execution_log", [])
        }
    ...
    def audit(self, trace_id: str, recorder) -> dict:
        ...
        return {
            ...
            "governance_approved": any(
                ...
                for d in trace.get("governance_decisions", [])
            ),
