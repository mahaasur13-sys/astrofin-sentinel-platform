    ...
    def execute(self, dag: dict, context: dict) -> dict:
        ...
        if self.governance_gate:
            try:
                ...
                trace["governance_decisions"].append(
                    {"stage": "pre", "decision": decision, "reason": reason}
                )
                ...
                trace["governance_decisions"].append(
                    {"stage": "pre", "decision": "ERROR", "reason": str(e)}
                )
    ...
    def _execute_node(self, node: dict, trace: dict, context: dict) -> tuple[bool, str]:
        try:
            if self.governance_gate and node.get("layer") == "L8":
                try:
                    if not self.governance_gate.mid_check(
                        node.get("node_id", ""), context
                    ):
                        trace["risk_events"].append(
                            {"node_id": node.get("node_id"), "event": "kill_switch"}
                        )
