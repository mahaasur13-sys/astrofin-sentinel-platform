...
class DAGNode:
    ...
    retry_policy: dict[str, Any] = field(
        default=lambda: {"max_retries": 3, "backoff": 2.0}
    )
...
class DAGCompiler:
    ...
    def _compile_agent_job(self, job: dict) -> dict:
        ...
        backend = (
            ExecutionBackend.GPU if agent_type in gpu_agents else ExecutionBackend.CPU
        )
        node = DAGNode(
    ...
    def _compile_risk_job(self, job: dict) -> dict:
        dag_id = str(uuid.uuid4())[:12]
        node = DAGNode(
            name="risk:analysis", node_type=NodeType.RISK, layer=Layer.L8, priority=100
        )
        return {
            "dag_id": dag_id,
            "nodes": [node.to_dict()],
        ...
        for node in dag["nodes"]:
            for inp in node.get("inputs", []):
                if inp not in node_ids and inp not in dag.get("external_inputs", []):
                    errors.append(
                        f"Node {node['node_id']} references unknown input: {inp}"
                    )
