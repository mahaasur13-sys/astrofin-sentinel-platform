...
class CorrectionLoop:
    ...
    def _detect_signals(self, metrics) -> list[CorrectionSignal]:
        ...
        # Drift alignment
        if (
            hasattr(metrics, "drift_alignment_error")
            and metrics.drift_alignment_error > 0.15
        ):
            signals.append(
                CorrectionSignal(
    ...
    def _decide(
        self, signals: list[CorrectionSignal], fix_type: FixType
    ) -> CorrectionDecision:
        ...
        primary = actions_map.get(
            worst.metric_name, CorrectionAction.ADJUST_QUEUE_DEPTH
        )
        ...
        if fix_type == FixType.POLICY:
            ...
            secondary.extend(
                [CorrectionAction.RELAX_CONSTRAINT, CorrectionAction.ESCALATE_HUMAN]
            )
    ...
    def _governance_approval(
        self, decision: CorrectionDecision, metrics
    ) -> CorrectionDecision:
        ...
        try:
            ...
            request = {
                ...
                "severity": max(
                    s.severity for s in getattr(decision, "signals", [object()])
                ),
                "metrics": {
    ...
    def _act(self, decision: CorrectionDecision) -> bool:
        ...
        self._correction_log.info(
            f"Acting: {decision.primary_action.value} ({decision.reason})"
        )

        # In real system: call scheduler API, update policy weights, etc.
        # Here: just log
        if decision.primary_action == CorrectionAction.TRIGGER_ROLLBACK:
            self._correction_log.warning(
                "Rollback triggered — would restore previous snapshot"
            )
            return True
    ...
    def _validate(
        self, signals: list[CorrectionSignal], decision: CorrectionDecision
    ) -> bool:
    ...
    def get_stats(self) -> dict:
        ...
        fixes = [
            ...
            if r.decision
            and r.decision.primary_action != CorrectionAction.ADJUST_QUEUE_DEPTH
        ]
