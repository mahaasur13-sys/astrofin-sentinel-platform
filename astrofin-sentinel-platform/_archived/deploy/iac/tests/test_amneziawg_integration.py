...
def test_awg_deterministic_delay():
    ...
    ok = delay1 == delay2
    log.info(
        f"  [OK{'=' if ok else '!'}] INV-AWG2 — Delay deterministic: {delay1:.4f} == {delay2:.4f}"
    )
    return ok
...
def test_awg_idempotent_start():
    ...
    log.info(
        f"  [OK{'=' if ok else '!'}] INV-AWG3 — Idempotent start: events={events_before}"
    )
    return ok
...
def test_awg_status_read_only():
    ...
    log.info(
        f"  [OK{'=' if ok else '!'}] INV-AWG4 — status() read-only: delta_events={count_after - count_before}"
    )
    return ok
...
def test_inv1_action_produces_event():
    ...
    log.info(
        f"  [OK{'=' if ok else '!'}] INV1 — Events: {len(all_events)}, returned: {returned}"
    )
    return ok
...
def test_inv6_trace_index_o1():
    ...
    log.info(
        f"  [OK{'=' if ok else '!'}] INV6 — O(1) lookup: {elapsed:.4f}s for 1000 lookups (100 traces)"
    )
    return ok
...
def test_inv7_projection_separation():
    ...
    log.info(
        f"  [OK{'=' if ok else '!'}] INV7 — Separation: raw={len(raw)}, state={state['status']}"
    )
    return ok
...
def test_patch1a_network_validation():
    ...
    log.info(
        f"  [OK{'=' if ok else '!'}] PATCH 1a — Network validation: valid={ok_valid}, dup={ok_dup}"
    )
    return ok
