...
log.info(f"(a) _chrom_to_vec loop: {(time.perf_counter()-t0)*1000:.1f} ms")

# (b) hasattr per candidate
t0 = time.perf_counter()
flags = [hasattr(c.strategy, "chromosome") for c in cands]
log.info(f"(b) hasattr loop: {(time.perf_counter()-t0)*1000:.1f} ms")
...
nn.fit(M)
log.info(f"(d1) nn.fit: {(time.perf_counter()-t0)*1000:.1f} ms")

Q = np.stack(vs)
t0 = time.perf_counter()
d, _ = nn.kneighbors(Q)
log.info(f"(d2) nn.kneighbors: {(time.perf_counter()-t0)*1000:.1f} ms")
...
log.info(f"(e) post filter: {(time.perf_counter()-t0)*1000:.1f} ms, kept={len(kept_idx)}")

# Full call again
t0 = time.perf_counter()
pool.diversity_filter(cands)
log.info(f"full call: {(time.perf_counter()-t0)*1000:.1f} ms")
