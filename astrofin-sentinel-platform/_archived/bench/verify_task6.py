...
log.info("hash present:", hasattr(ss, "_chromosome_hash"))
log.info("hash len:", len(ss._chromosome_hash), "(16=hex_sha256_prefix)")
log.info("hash stable:", ss._chromosome_hash == ss._chromosome_hash)
...
log.info(
    f"diversity_filter (NN) n=1000 cands vs n=1000 pool: {dt*1000:.1f} ms, kept {len(kept)}/1000"
)
log.info(f"DoD <100ms for 1000 candidates: {dt < 0.1}")

# --- Opt 3: downsample_equity_curve ---
ec = np.linspace(100.0, 200.0, 50000)
ds = downsample_equity_curve(ec, max_points=1000)
log.info(f"downsample 50000 -> {len(ds)} (target 1000)")
ec_short = np.array([1.0, 2.0, 3.0])
log.info(
    f"downsample short array passthrough: len={len(downsample_equity_curve(ec_short))}"
)

# Edge: non-array input
log.info(f"downsample list input: len={len(downsample_equity_curve([1.0,2.0,3.0,4.0]))}")
