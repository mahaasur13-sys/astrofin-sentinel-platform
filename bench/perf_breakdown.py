...
for i in range(N):
    ...
    ss = ScoredStrategy(strategy=st, reward=0.5 + i * 0.0001, evaluation=EvaluationResult.fail())
    pool.add(ss)
    cands.append(ScoredStrategy(strategy=GeneratedStrategy(random_chromosome(), generation=1), reward=0.5, evaluation=EvaluationResult.fail()))
