...
def stub_client() -> EmbeddingClient:
    ...
    return EmbeddingClient(
        EmbeddingConfig(provider="stub", dimension=1536, cache_enabled=True)
    )
...
def no_cache_client() -> EmbeddingClient:
    return EmbeddingClient(
        EmbeddingConfig(provider="stub", dimension=1536, cache_enabled=False)
    )
...
async def test_batch_chunks_correctly(stub_client):
    small_batch_client = EmbeddingClient(
        EmbeddingConfig(
            provider="stub", dimension=1536, max_batch_size=2, cache_enabled=False
        )
    )
    ...
    with patch.object(
        ...
        wraps=small_batch_client._deterministic_vector,
    ) as mock_det:
...
async def test_cache_ttl_expiry(stub_client):
    ...
    assert (
        "expiring text" in [stub_client._cache[k][0] for k in stub_client._cache]
        or len(stub_client._cache) == 1
    )
    # Advance time beyond TTL
    with patch(
        "tools.embedding_client.time.monotonic", return_value=time.monotonic() + 100
    ):
...
async def test_openai_rate_limit_retries_then_raises():
    ...
    c._openai_client.embeddings.create = AsyncMock(
        side_effect=openai.RateLimitError(
            "rate limited", response=AsyncMock(status_code=429), body=None
        )
    )
    with patch("tools.embedding_client.asyncio.sleep", new=AsyncMock()) as mock_sleep:
        with pytest.raises(
            RuntimeError, match="OpenAI embedding failed after 2 attempts"
        ):
            await c._embed_openai(["test"])
