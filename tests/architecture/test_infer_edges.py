...
def _write_fake_report(edges: list[dict], target: Path) -> None:
    ...
    for i, e in enumerate(edges, 1):
        ...
        lines.append(
            f"- **Source:** `{e['source_path']}:L{e['source_line']} :: {e['source_node_id']}`"
        )
        lines.append(
            f"- **Target:** `{e['target_path']}:L{e['target_line']} :: {e['target_node_id']}`"
        )
        lines.append(
...
def _run_infer_edges(workspace: Path, out: Path) -> dict:
    ...
    src = src.replace(
        'REPO_ROOT = Path("/home/workspace")', f'REPO_ROOT = Path("{workspace}")'
    )
    src = src.replace(
...
def test_summary_writes_when_json_format(fake_workspace, tmp_path):
    ...
    src = INFER_EDGES.read_text()
    src = src.replace(
        'REPO_ROOT = Path("/home/workspace")', f'REPO_ROOT = Path("{fake_workspace}")'
    )
    src = src.replace(
        'GRAPH_JSON = REPO_ROOT / "graphify-out" / "graph.json"',
        'GRAPH_JSON = REPO_ROOT / "graph.json"',
