...
def _make_strategy(chromosome: dict | None = None, generation: int = 1) -> GeneratedStrategy:
...
class TestStrategyPoolUnit:
    ...
    def test_top_k_returns_strategies_sorted_by_reward(self):
        ...
        scored_list = (
            [_make_scored(reward=0.1, sid=f"low_{i}") for i in range(2)] + [_make_scored(reward=0.9, sid=f"hi_{i}") for i in range(2)] + [_make_scored(reward=0.5, sid=f"mid_{i}") for i in range(2)]
        )
...
class TestPersistenceScoredAndVersions:
    ...
    def test_save_version_load_version_list_compare(self):
        ...
        try:
            ...
            weak_pool = [_make_scored(reward=0.2, sid=f"a_{i}") for i in range(3)] + [_make_scored(reward=0.4, sid="a_3")]
            assert persistence.save_version(weak_pool, tags[0]) is True

            # Version B: stronger pool (mean reward 0.8).
            strong_pool = [_make_scored(reward=0.7, sid=f"b_{i}") for i in range(3)] + [_make_scored(reward=1.0, sid="b_3")]
