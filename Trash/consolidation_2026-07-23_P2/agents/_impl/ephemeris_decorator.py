...
def require_ephemeris(func: Callable[P, T]) -> Callable[P, T]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        if not HAS_SWISS_EPHEMERIS:
            owner = args[0].__class__.__name__ if args and hasattr(args[0], "__class__") else func.__qualname__
            raise EphemerisUnavailableError(f"'{owner}' requires Swiss Ephemeris. Install with: pip install pyswisseph")
