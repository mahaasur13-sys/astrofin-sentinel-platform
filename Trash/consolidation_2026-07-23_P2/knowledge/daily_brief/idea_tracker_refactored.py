#!/usr/bin/env python3
"""
idea_tracker.py — ATOM-R-041: Idea → Outcome Tracking
Refactored: dataclass + dict unified contract via core/idea_model.py
...
Usage:
    python knowledge/daily_brief/idea_tracker.py --list
    python knowledge/daily_brief/idea_tracker.py --pending
    python knowledge/daily_brief/idea_tracker.py --kpi
    python knowledge/daily_brief/idea_tracker.py --inject IDEA_ID
    python knowledge/daily_brief/idea_tracker.py --eval IDEA_ID --reward 0.15
    python knowledge/daily_brief/idea_tracker.py --add "text" --source manual --category GENERAL
...
# KARL buffer integration: accept Idea dict or Idea dataclass
# Decision record: serialize Idea via asdict() only
...
class Idea:
    ...
    def to_dict(self) -> dict:
        """Serialize to dict — only permitted output format."""
        return asdict(self)

    @staticmethod
    def from_dict(d: dict) -> "Idea":
        """Deserialize from dict — only permitted input format."""
        d.pop("_id", None)
        return Idea(**d)

    # Convenience: expose fields as dict keys for backward compat
    def get(self, key: str, default=None):
        """Allow idea['field'] access pattern (like dict)."""
        return getattr(self, key, default)

    def __getitem__(self, key: str):
        """idea['field'] dict-style access."""
        return getattr(self, key)

    def __setitem__(self, key: str, value):
        """idea['field'] = value dict-style mutation."""
        setattr(self, key, value)

    def keys(self):
        """dict.keys() compatibility."""
        return [
            "id",
            "source",
            "text",
