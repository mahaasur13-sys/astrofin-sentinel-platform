...
def get_price(symbol: str, timeframe: str = "1d") -> float | None:
...
def get_klines(symbol: str, interval: str = "1d", limit: int = 100) -> list[float]:
    ...
    try:
        url = f"https://api.binance.com/api/v3/klines" f"?symbol={symbol}&interval={interval}&limit={limit}"
        resp = requests.get(url, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
...
def get_market_cap(symbol: str) -> dict | None:
