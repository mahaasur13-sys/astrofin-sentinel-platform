...
class MetaLearner:
    ...
    def _all_time_best(self) -> list[PolicyRecommendation]:
        ...
        if not self._trials:
            return [
                PolicyRecommendation(
                    policy_id="default", weight=1.0, confidence=0.0, reason="no data"
                )
            ]

        avg_regrets = {
            pid: np.mean(scores) for pid, scores in self._policy_scores.items()
        }
    ...
    def get_best_policy(
        self, workload_type: str, cluster_state_class: str
    ) -> str | None:
