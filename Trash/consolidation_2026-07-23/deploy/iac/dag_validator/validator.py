...
class DAGValidator:
    ...
    def _serialize(self, obj, _seen=None):
        ...
        if isinstance(obj, (list, tuple)):
            return [self._serialize(x, _seen) for x in obj]
        if isinstance(obj, dict):
            return {k: self._serialize(v, _seen) for k, v in obj.items()}
    ...
    def validate(
        self, dag: dict[str, Any], deterministic_seed: int | None = None
    ) -> ValidationResult:
        ...
        if cycle_path:
            violations.append(
                Violation(
                    ViolationType.I1_CYCLE, None, f"Cycle: {' -> '.join(cycle_path)}"
                )
            )
        violations.extend(self._check_dependency_closure(nodes, edges))
        violations.extend(
            self._check_deterministic_order(nodes, edges, deterministic_seed)
        )
        ...
        result = ValidationResult(
            ...
            invariants_satisfied={
                ...
                "I2": not any(
                    v.type == ViolationType.I2_MISSING_INPUT for v in violations
                ),
                "I3": not any(
                    v.type == ViolationType.I3_NONDETERMINISTIC for v in violations
                ),
                "I4": not any(
                    v.type == ViolationType.I4_SIDE_EFFECT for v in violations
                ),
            },
        )
    ...
    def _check_deterministic_order(
        self, nodes: dict, edges: list, seed: int | None
    ) -> list[Violation]:
        import random
        ...
        if len(frontier) > 1:
            violations.append(
                Violation(
                    ViolationType.I3_NONDETERMINISTIC,
                    None,
