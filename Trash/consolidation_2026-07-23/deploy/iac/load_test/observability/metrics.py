...
class MetricsCollector:
    ...
    def check_slo_breaches(self, metrics: SystemMetrics) -> list[str]:
        ...
        if metrics.p99_latency_ms > self._thresholds.p99_latency_ms:
            breaches.append(
                f"p99_latency={metrics.p99_latency_ms:.0f}ms > {self._thresholds.p99_latency_ms}ms"
            )
        if metrics.failure_rate > self._thresholds.failure_rate:
            breaches.append(
                f"failure_rate={metrics.failure_rate:.3f} > {self._thresholds.failure_rate}"
            )
        ...
        if metrics.drift_alignment_error > self._thresholds.drift_error:
            breaches.append(
                f"drift_error={metrics.drift_alignment_error:.3f} > {self._thresholds.drift_error}"
            )
        if metrics.admission_reject_rate > self._thresholds.reject_rate:
            breaches.append(
                f"reject_rate={metrics.admission_reject_rate:.3f} > {self._thresholds.reject_rate}"
            )
        return breaches
