...
class ConstraintGraph:
    ...
    def add_anti_affinity_constraint(
        self, job_id: str, forbidden_node: str, reason: str = ""
    ):
    ...
    def add_temporal_constraint(
        self, job_id: str, earliest_start: float, migration_delay: float = 0.0
    ):
    ...
    def add_sla_constraint(
        self, job_id: str, max_latency: float, target_p: float = 0.05
    ):
    ...
    def get_node_constraints(self, node_id: str) -> list[Constraint]:
        ...
        return [c for c in self.edges if c.source == node_id or c.target == node_id]
    ...
    def validate_placement(self, job_id: str, node_id: str) -> tuple[bool, list[str]]:
        ...
        affinity_edges = [
            c
            for c in self.edges
            if c.ctype == ConstraintType.AFFINITY and c.source == job_id
        ]
        ...
        anti_edges = [
            c
            for c in self.edges
            if c.ctype == ConstraintType.ANTI_AFFINITY
            and c.source == job_id
